...
Title.Text = "قائمة التحكم (الرادار الذكي)"
...
-- إحداثيات نقاط الممرات الستة (بدايات ونهايات الممرات الثلاثة)
local aislePoints = {
...
-- دالة التحرك (محمية ضد النزول تحت الأرض)
...
-- البحث عن الهدف الحقيقي في المتجر
...
-- اختيار أقرب إحداثي من الممرات الستة بناءً على موقع الهدفك المكتشف
    ...
    task.spawn(function()
        ...
        while autoFarm do
            if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                ...
                -- إذا كان اللاعب بعيد، يدخله ويوظفه
                ...
                -- 2. الذهاب لمنتصف المتجر (نقطة 7) كمحطة رادار للفحص
                moveToPosition(hrp, waypoints[7], speed)
                if not autoFarm then break end
                task.wait(0.3)
                
                -- 3. استخدام الرادار: كشف موقع التعبئة واختيار أقرب نقطة ممر آمنة له
                local realDeliveryPoint = findHiddenDeliveryTarget()
                if realDeliveryPoint then
                    -- يختار الإحداثي الصح من الـ 6 نقاط (بداية/نهاية الممرات)
                    local safeAisleTarget = getClosestAislePoint(realDeliveryPoint)
                    
                    -- يروح دايركت للنقطة الصحيحة بدون اختراق
                    moveToPosition(hrp, safeAisleTarget, speed)
                    if not autoFarm then break end
                    task.wait(0.2)
                    
                    -- تفريغ الصناديق في المكان الصحيح
                    for _ = 1, 15 do
                        if not autoFarm then break end
                        pressEKey()
                        task.wait(0.15)
                ...
                -- راحة خفيفة قبل تكرار جلب صناديق جديدة
                task.wait(0.5)
