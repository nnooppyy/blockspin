...
----------------------------------------------------
-- التعديل الجديد لمنطق الـ Auto Respawn 
----------------------------------------------------
...
task.spawn(function()
    ...
    while true do
        task.wait(0.5) -- المراقبة كل نصف ثانية
        if autoRespawnEnabled then
            pcall(function()
                local targetButton = nil
                
                -- 1. البحث عن الزر اللي يحمل كلمة Respawn
                for _, gui in ipairs(PlayerGui:GetDescendants()) do
                    if (gui:IsA("TextButton") or gui:IsA("ImageButton")) then
                        ...
                        if string.find(name, "respawn") or string.find(text, "respawn") then
                            -- التأكد المطلق أن الزر وكل القوائم التي فوقه مرئية (عشان ما يضغط بالغلط)
                            local isTrulyVisible = true
                            local currentObj = gui
                            while currentObj and currentObj:IsA("GuiObject") do
                                if not currentObj.Visible then
                ...
                -- 2. إذا لقاه وكان ظاهر بالشاشة، بيقعد يضغط بشكل متكرر لحد ما يختفي
                if targetButton then
                    while autoRespawnEnabled and targetButton.Parent and targetButton.Visible do
                        -- محاولة 1: ضغط برمجياً عبر دوال اللعبة (فعال 90%)
                        pcall(function()
                        ...
                        -- محاولة 2: محاكاة ضغطة الماوس الفعلية بالمنتصف 
                        local absPos = targetButton.AbsolutePosition
                        local absSize = targetButton.AbsoluteSize
                        local inset, _ = GuiService:GetGuiInset() -- تعويض شريط روبلوكس العلوي
                        ...
                        task.wait(0.2) -- يضغط كل 0.2 ثانية لين يختفي تماماً
                    end
                end
            end)
        end
