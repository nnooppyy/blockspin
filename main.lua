...
-- فحص هل اللاعب يحمل أداة أو صندوق في الحقيبة
local function hasToolOrBox()
    local player = game.Players.LocalPlayer
    if player.Character and player.Character:FindFirstChildOfClass("Tool") then
        return true
    end
    if player:FindFirstChild("Backpack") and player.Backpack:FindFirstChildOfClass("Tool") then
        return true
    end
    return false
end

-- البحث الذكي عن مكان السهم للتوجه والتعبئة
    ...
    task.spawn(function()
        ...
        if player.Character and player.Character:FindFirstChild("HumanoidRootPart") and autoFarm then
            ...
            for i = startIndex, #waypoints do
                ...
                -- إذا وصل للنقطة 5 وكان يملك صندوقاً بالفعل أو أداة، يتخطاها
                if i == 5 and hasToolOrBox() then
                    continue
                end
                ...
                -- تنفيذ الأوامر الخاصة بكل نقطة
                if i == 5 then
                    -- النقطة 5: التوظف المؤكد
                    task.wait(0.2)
