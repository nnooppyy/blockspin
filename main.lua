...
-- حركة طبيعية تماماً باستخدام مشي الشخصية الحقيقي (بدون أي تيليبرورت أو سرعة مفاجئة)
local function moveToPosition(hrp, targetPos, speed)
    ...
    local timeout = tick() + 15 -- وقت إضافي كافي للوصول بدون تعليق
    repeat
        task.wait(0.1)
        checkAndUnsit(humanoid)
    ...
    until (hrp.Position - safePos).Magnitude < 3 or tick() > timeout
end
