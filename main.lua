...
local waypoints = {
    ...
    Vector3.new(164.69, 254.99, 202.29), -- 6
    Vector3.new(148.91, 254.74, 208.63), -- 7
    Vector3.new(125.54, 254.74, 201.89), -- 8
    Vector3.new(203.44, 254.94, 209.60), -- 9
    Vector3.new(205.31, 254.95, 263.65)  -- 10
}
...
-- وظيفة فحص رسالة "Teleport detected"
...
-- وظيفة الانتقال (تم تحديثها لدعم إيقاف الـ Tween عند ظهور رسالة التليپورت)
local function moveToPosition(hrp, targetPos, speed)
    if not autoFarm then return end
    
    -- الانتظار إذا كانت الرسالة موجودة قبل بدء الحركة
    ...
    repeat
        task.wait(0.05)
        -- فحص مستمر لرسالة الحماية أثناء الحركة
        if isTeleportDetected() then
            ...
            expectedEnd = expectedEnd + pauseDuration -- تعويض الوقت الضائع
            if autoFarm then
                tween:Play()
            end
        end
...
-- وظيفة جلب أقرب نقطة من النقاط الـ 3 الأولى فقط (لضمان الدخول من مسار الشارع الصحيح)
    ...
    task.spawn(function()
        while autoFarm do
            ...
            if localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                local distToBoxArea = (hrp.Position - waypoints[7]).Magnitude
                
                -- إذا ما عنده صندوق، يروح يجيب بالمسار الصحيح (يقيس أقرب نقطة من 1 إلى 3 ويكمل لـ 7)
                if not playerHasBox() then
                    -- إذا كان اللاعب برا أو بعيد عن منطقة الصناديق
                    if distToBoxArea > 40 or status == "unemployed" or status == "unknown" then
                        local startIndex = getClosestStartPoint(hrp.Position)
                        for i = startIndex, 7 do
                            if not autoFarm then break end
                            moveToPosition(hrp, waypoints[i], speed)
                            
                            if i == 6 and (status == "unemployed" or status == "unknown") then
                                task.wait(0.3)
                                clickCenter()
                                task.wait(0.3)
                            end
                        ...
                        -- إذا كان أصلاً قريب من الصناديق
                ...
                if playerHasBox() then
                    ...
                    if realDeliveryPoint then
                        ...
                        -- التوجه إلى منتصف نقطة التعبئة بالضبط وبدون أي مسافة خارجية
