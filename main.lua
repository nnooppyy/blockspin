...
local waypoints = {
    ...
    Vector3.new(164.69, 254.99, 202.29), -- 6 (نقطة التوظف)
...
-- دالة لتحديد أقرب نقطة داخل البقالة (النقاط 7 و 8 مثلاً) ثم العودة تنازلياً حتى النقطة 6
local function getClosestGroceryWaypointIndex(playerPos)
    local closestIndex = 7
    local minDistance = math.huge
    -- نفحص النقاط الموجودة داخل البقالة فقط (نقطة 7 ونقطة 8)
    for i = 7, #waypoints do
        local dist = (playerPos - waypoints[i]).Magnitude
        if dist < minDistance then
            minDistance = dist
            closestIndex = i
            ...
            if not playerHasBox() then
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if isInsideGrocery(hrp.Position) then
                        -- إذا كان داخل البقالة، يذهب لأقرب نقطة داخل البقالة (مثلاً 8 أو 7)
                        local startIdx = getClosestGroceryWaypointIndex(hrp.Position)
                        
                        -- يمر عبر النقاط تنازلياً حتى يصل إلى النقطة 6 (مثلاً: 8 -> 7 -> 6)
                        for i = startIdx, 6, -1 do
                            if not autoFarm then break end
                            moveToPosition(hrp, waypoints[i], speed)
                        end
                    else
                        -- إذا كان خارج البقالة، يتبع المسار الطبيعي من 1 إلى 6
