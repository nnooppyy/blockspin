...
-- دالة فحص زر الوظيفة (Apply إذا مو متوظف، أو Leave إذا متوظف)
local function checkAndApplyJobIfNeeded()
    pcall(function()
        ...
        -- إذا مو متوظف، يضغط مرة وحدة فقط
        if needsApply then
            clickCenter()
            task.wait(0.3)
        end
                ...
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if not isDepositing then
                        if not isInsideGrocery(hrp.Position) then
                            ...
                            -- اللاعب جوا البقالة (يمشي 9 ثم 8 ثم 7 ويفحص الوظيفة مرة وحدة)
                            moveToPositionHelper(hrp, waypoints[9], speed)
