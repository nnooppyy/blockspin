...
----------------------------------------------------
-- 3. محتوى قسم الأخرى (Other) ويحتوي على زر Respawn
----------------------------------------------------
...
RespawnToggleButton.Parent = OtherContent
RespawnToggleButton.Size = UDim2.new(0.95, 0, 0, 40)
RespawnToggleButton.BackgroundColor3 = Color3.fromRGB(180, 50, 50)
RespawnToggleButton.BackgroundTransparency = 0.2
RespawnToggleButton.Text = "💀 Auto Respawn: [OFF]"
RespawnToggleButton.TextColor3 = Color3.fromRGB(255, 255, 255)
RespawnToggleButton.TextSize = 12
RespawnToggleButton.Font = Enum.Font.GothamBold
RespawnToggleButton.ZIndex = 52

local RespawnCorner = Instance.new("UICorner")
RespawnCorner.CornerRadius = UDim.new(0, 6)
RespawnCorner.Parent = RespawnToggleButton
...
----------------------------------------------------
-- منطق الـ Auto Respawn (تمت زيادة المسافة للأسفل أكثر لتحديد زر الـ Respawn بدقة)
----------------------------------------------------
local autoRespawnEnabled = false

RespawnToggleButton.MouseButton1Click:Connect(function()
    autoRespawnEnabled = not autoRespawnEnabled
    if autoRespawnEnabled then
        RespawnToggleButton.Text = "💀 Auto Respawn: [ON]"
        RespawnToggleButton.BackgroundColor3 = Color3.fromRGB(50, 150, 80)
    else
        RespawnToggleButton.Text = "💀 Auto Respawn: [OFF]"
        RespawnToggleButton.BackgroundColor3 = Color3.fromRGB(180, 50, 50)
    end
...
-- حلقة مراقبة زر الرسبون وانتظار انتهاء العداد واختفاء الأرقام
task.spawn(function()
            ...
            if pGui then
                for _, descendant in ipairs(pGui:GetDescendants()) do
                    if (descendant:IsA("TextButton") or descendant:IsA("TextLabel")) and descendant.Visible then
                        ...
                        -- التحقق من وجود كلمة respawn وأنه لا توجد أرقام عداد بداخل النص
                        if string.find(text, "respawn") and not string.find(text, "%d") then
                            local targetButton = nil
                            if descendant:IsA("TextButton") then
                                targetButton = descendant
                            else
                                local parentBtn = descendant.Parent
                            ...
                            if targetButton and targetButton.Visible then
                                local abPos = targetButton.AbsolutePosition
                                local abSize = targetButton.AbsoluteSize
                                local centerX = abPos.X + (abSize.X / 2)
                                -- تم زيادة القيمة هنا إلى +85 للنزول أكثر لمنتصف زر الـ Respawn تماماً
                                local centerY = abPos.Y + (abSize.Y / 2) + 85
                                
                                -- الضغط مرة واحدة في الموضع الجديد الصحيح
                                VirtualInputManager:SendMouseButtonEvent(centerX, centerY, 0, true, game, 0)
                                task.wait(0.05)
                                VirtualInputManager:SendMouseButtonEvent(centerX, centerY, 0, false, game, 0)
                                
                                -- الانتظار قليلاً حتى يتم الرسبون وتختفي القائمة لتجنب التكرار السريع
                                task.wait(3)
                            end
                        end
                    end
...
----------------------------------------------------
-- المنطق والإحداثيات (باقي الكود الأصلي)
----------------------------------------------------
local waypoints = {
