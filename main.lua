...
----------------------------------------------------
-- القائمة الجانبية (يسار) وممنطقة المحتوى (يمين)
----------------------------------------------------
...
----------------------------------------------------
-- الإحداثيات والمنطق البرمجي المتقدم
----------------------------------------------------
local waypoints = {
...
-- فحص الأداة/الصندوق وإرجاعه لليد فوراً إذا سقط بسبب الضرب أو البقس
...
-- نظام الحركة مع الحماية التلقائية ضد الطرد (البقس أو الدذف برة الدائرة)
local function moveToPosition(hrp, targetPos, speed)
    ...
    repeat
        task.wait(0.05)
        checkAndUnsit(humanoid)
        ensureBoxEquipped() -- ضمان تشغيل/حمل الصندوق فوراً لو انطردت
        
        -- التحقق إذا اللاعب انطرد مسافة كبيرة برة المنطقة (بسبب البقس) وإرجاعه فوراً
        if (hrp.Position - safePos).Magnitude > 15 and autoFarm then
            tween:Cancel()
            task.wait(0.05)
            hrp.CFrame = CFrame.new(safePos)
            ensureBoxEquipped()
            
            -- إعادة تشغيل التوين للمسافة المتبقية
            distance = (hrp.Position - safePos).Magnitude
            tweenTime = distance / math.max(speed, 1)
            ...
            while autoFarm do
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    while autoFarm and playerHasBox() do
                        ensureBoxEquipped() -- تفعيل الصندوق وتثبيته في اليد
                        ...
                        if realDeliveryPoint then
                            ...
                            while autoFarm and isTargetStillActive(realDeliveryPoint) do
                                checkAndUnsit(humanoid)
                                ensureBoxEquipped() -- ضمان حمل الصندوق حتى لو انضربت أثناء التسليم
                                task.wait(0.3)
                            end
                            ...
                            moveToPosition, waypoints[8], speed -- تم التعديل لتصحيح النحو
                            if not autoFarm then break end
                        else
                            task.wait(1)
                            break
