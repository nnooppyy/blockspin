...
local function moveToPosition(hrp, targetPos, speed)
    if not autoFarm or not hrp or not hrp.Parent then return end
    ...
    if not autoFarm and tween then tween:Cancel() end
end
                ...
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    while autoFarm and playerHasBox() do
                        ...
                        if realDeliveryPoint then
                            ...
                            -- حلقة مراقبة التبقيص: إذا تم دفعه للخارج أو تأثر، يعود للنص ويستمر في استخدام الصندوق
                            while autoFarm and isTargetStillActive(realDeliveryPoint) do
                                checkAndUnsit(humanoid)
                                ensureBoxEquipped()
                                
                                -- فحص المسافة بين اللاعب وهدف التسليم (إذا ابتعد أكثر من 5 متر يعني تم دفعه للخارج)
                                if (hrp.Position - realDeliveryPoint).Magnitude > 5 then
                                    moveToPosition(hrp, Vector3.new(realDeliveryPoint.X, hrp.Position.Y, realDeliveryPoint.Z), speed)
                                end
