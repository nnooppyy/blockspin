...
----------------------------------------------------
-- منطق التنقل والحركة (Farm Logic للمسارات المباشرة)
----------------------------------------------------
            ...
            while autoFarm do
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if not isInsideGrocery(hrp.Position) then
                        -- البحث عن أقرب نقطة في المسار أينما كان موقع اللاعب
                        ...
                        -- التحرك من أقرب نقطة تم تحديدها تدريجياً وإكمال باقي النقاط بالترتيب
                        local targetEndIndex = #waypoints - 2
                        if closestIndex <= targetEndIndex then
                            for i = closestIndex, targetEndIndex do
                                if not autoFarm then break end
                                moveToPosition(hrp, waypoints[i], speed)
                            end
                        else
                            moveToPosition(hrp, waypoints[targetEndIndex], speed)
                        end
