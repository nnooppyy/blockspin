...
local RunService = game:GetService("RunService") -- تمت إضافته لاختراق الجدران
...
local hasCheckedEmployment = false
local pointsFolder = nil
local VirtualInputManager = game:GetService("VirtualInputManager")
local noclipConnection = nil -- تمت إضافته لاختراق الجدران
...
-- الحركة الثابتة عبر الـ CFrame اللي ضبطناها بالرد السابق
local function moveToPosition(hrp, targetPos, speed)
    ...
    while autoFarm do
        ...
        local direction = (safePos - currentPos)
        local distance = direction.Magnitude
        
        -- إذا وصل للنقطة يطلع من اللوب
        if distance <= 0.5 then
            break
        end
        
        -- حساب المسافة اللي بيمشيها بهاللحظة عشان ما يطير فجأة
        local step = speed * dt
        if step > distance then
            step = distance
        end
        
        -- تحريك الشخصية بالـ CFrame بشكل ثابت وناعم
        ...
        while autoFarm do
            ...
            if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if not playerHasBox() then
                    if not hasCheckedEmployment then
