-- التأكد من تحميل اللاعب والـ PlayerGui (مرة واحدة عند تشغيل السكربت)
...
-- ✅ تعديل: إضافة مهلة قصوى لحلقة كشف التليبورت
...
local function waitForTeleportClear()
    local startTime = tick()
    while isTeleportDetected() and autoFarm do
        task.wait(0.2)
        if tick() - startTime > 5 then -- لو استمرت الرسالة أكثر من 5 ثواني نتجاهلها
            break
        end
    end
end
...
local function moveToPosition(hrp, targetPos, speed)
    ...
    waitForTeleportClear() -- استخدام الدالة الجديدة
    ...
    repeat
        ...
        if isTeleportDetected() then
            tween:Pause()
            waitForTeleportClear()
            if autoFarm then tween:Play() end
            -- نعيد حساب الوقت المتبقي
            local remaining = (hrp.Position - safePos).Magnitude / math.max(speed, 1)
            expectedEnd = tick() + remaining
        end
...
-- ✅ تحسين بسيط: تقليل نطاق البحث في workspace باستخدام Region3
local function findHiddenDeliveryTarget()
    ...
    local region = Region3.new(searchCenter - Vector3.new(searchRadius, 50, searchRadius), searchCenter + Vector3.new(searchRadius, 50, searchRadius))
    local partsInRegion = workspace:FindPartsInRegion3(region, nil, 500)
    
    local targetPos = nil
    local minDist = searchRadius
...
local function isTargetStillActive(targetPos)
    ...
    local region = Region3.new(targetPos - Vector3.new(searchRadius, 10, searchRadius), targetPos + Vector3.new(searchRadius, 10, searchRadius))
...
FarmButton.MouseButton1Click:Connect(function()
    ...
    if autoFarm then
        ...
        -- ✅ التحقق من الشخصية مرة واحدة فقط
