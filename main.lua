...
FarmButton.MouseButton1Click:Connect(function()
    ...
    if autoFarm then
        ...
        -- **طريقة نبضات الشفت المتجددة (تحل مشكلة تعليق التوقف بعد فترة)**
        task.spawn(function()
            while autoFarm do
                pcall(function()
                    -- ضغط نظيف ومتجدد: تشغيل ثم إيقاف ثم تشغيل جديد لمنع تعليق المفتاح
                    VirtualInputManager:SendKeyEvent(false, Enum.KeyCode.LeftShift, false, game)
                    task.wait(0.05)
                    VirtualInputManager:SendKeyEvent(true, Enum.KeyCode.LeftShift, false, game)
                end)
                task.wait(0.4) -- يتجدد كل 0.4 ثانية لضمان استمراره دون أن يعلق النظام
            end
            -- عند الإيقاف، يحرر الزر نهائياً
            pcall(function()
                VirtualInputManager:SendKeyEvent(false, Enum.KeyCode.LeftShift, false, game)
            end)
        end)
