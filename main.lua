...
----------------------------------------------------
-- منطق التنقل والحركة وإيداع الأموال (بالمبلغ الكامل الحالي)
----------------------------------------------------
...
local function checkShouldDeposit()
    return autoDepositEnabled and (getPlayerCash() >= targetDepositAmount)
end

-- =========================================================================
-- مسار الإيداع المطلوب: نقطة 9 -> رف S3 -> البنك (مع إيداع المبلغ كاملاً)
-- =========================================================================
local function executeDepositSequence(speed)
    ...
    -- الضغط على زر Deposit
    clickButtonByKeyword("deposit")
    task.wait(0.6)
    
    -- قراءة المبلغ الحقيقي بالكامل وكتابته في الخانة
    local fullCash = getPlayerCash()
    if fullCash <= 0 then
        fullCash = targetDepositAmount
    end
    
    typeInTextBox(fullCash)
    task.wait(0.5)
    
    -- تأكيد الإيداع بالمبلغ الكامل
        ...
        task.spawn(function()
            while autoFarm do
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    -- فحص الإيداع وترك كل شيء للتوجه فوراً حسب الترتيب المطلوب
                    if checkShouldDeposit() and isInsideGrocery(hrp.Position) and not isDepositing then
                        isDepositing = true
                    ...
                    if not isDepositing then
                            ...
                            if distToPreStart <= proximityThreshold or distToPoint0 <= proximityThreshold then
                                ...
                                if autoFarm then
                                    for i = 4, targetEndIndex do
                                        if not autoFarm or checkShouldDeposit() then break end
                        ...
                        if not playerHasBox() then
                            ...
                            for _ = 1, 8 do
                                if not autoFarm or playerHasBox() or checkShouldDeposit() then break end
                                pressEKey()
                                task.wait(0.2)
                            end
                            task.wait(0.3)
                        ...
                        while autoFarm and playerHasBox() and not isDepositing do
                            if checkShouldDeposit() then break end
