...
----------------------------------------------------
-- منطق التنقل والحركة وإيداع الأموال (سريع جداً في الإيداع)
----------------------------------------------------
...
function moveToPositionHelper(hrp, targetPos, speed)
    ...
    while isTeleportDetected() and autoFarm do task.wait(0.05) end
...
-- التحقق عما إذا كان الصندوق نشطاً ومستخدماً حالياً
...
-- =========================================================================
-- مسار الإيداع (سريع جداً في التفاعل الفوري مع الصراف)
-- =========================================================================
local function executeDepositSequence(speed)
    ...
    -- التفاعل الفوري والسريع مع الصراف بدون أي انتظار طويل
    pressEKey()
    task.wait(0.05)
    pressEKey()
    task.wait(0.1)
    
    -- الضغط السريع لفتح خانة الإيداع
    clickAtScreenPosition(372, 162)
    task.wait(0.08)
    
    -- جلب الرصيد وإيداعه فوراً
    ...
    -- الضغط السريع على زر التأكيد
    clickAtScreenPosition(366, 196)
    task.wait(0.1)
    
    -- العودة السريعة لمسار الفارم
    moveToPositionHelper(hrp, aislePoints[3], speed)
    moveToPositionHelper(hrp, waypoints[9], speed)
end
                    ...
                    if not isDepositing then
                        if not isInsideGrocery(hrp.Position) then
                            ...
                            if distToPreStart <= proximityThreshold or distToPoint0 <= proximityThreshold then
                                ...
                                if autoFarm then task.wait(0.05) clickCenter() task.wait(0.15) end
                        ...
                        while autoFarm and playerHasBox() and not isDepositing do
                            ...
                            -- التحقق قبل البيع: إذا لم يكن الصندوق مستخدماً/مجهزاً، اضغط على الرقم 2 فوراً
