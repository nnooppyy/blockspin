            ...
            while autoFarm do
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if not isInsideGrocery(hrp.Position) then
                        -- التحقق: إذا كان اللاعب قريب من النقطة رقم 1 (أقل من 60 ستود)
                        local distToPoint1 = (hrp.Position - waypoints[1]).Magnitude
                        if distToPoint1 <= 60 then
                            -- يمشي مع 1 ثم 2 ثم 3 بالترتيب
                            for i = 1, 3 do
                                if not autoFarm then break end
                                moveToPosition(hrp, waypoints[i], speed)
                            end
                            -- ثم يكمل باقي طريقه من 4 إلى آخره
                            for i = 4, #waypoints - 2 do
                                if not autoFarm then break end
                                moveToPosition(hrp, waypoints[i], speed)
                            end
                        else
                            -- إذا لم يكن قريباً من النقطة 1، يبدأ مباشرة من النقطة 4 وتوابعها
                            if not autoFarm then break end
