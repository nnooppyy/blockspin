        ...
        task.spawn(function()
            if not playerHasBox() then
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    -- التحقق مما إذا كان اللاعب داخل حدود البقالة
                    if isInsideGrocery(hrp.Position) then
                        -- إيجاد أقرب نقطة من الـ waypoints للاعب داخل البقالة
                        local closestIdx = 1
                        local minDst = math.huge
                        for i, pt in ipairs(waypoints) do
                            local dst = (hrp.Position - pt).Magnitude
                            if dst < minDst then
                                minDst = dst
                                closestIdx = i
                            end
                        ...
                        -- التحرك إلى أقرب نقطة تم رصدها
                        moveToPosition(hrp, waypoints[closestIdx], speed)
                        
                        -- إكمال التحرك تنازلياً نحو النقاط التي قبلها إذا لزم الأمر (مثل 8 ثم 7 ثم 6 إلخ)
                        for i = closestIdx - 1, 1, -1 do
