...
-- القائمة الرئيسية (تم تكبير الارتفاع قليلاً لتسع خانة الفلوس)
...
----------------------------------------------------
-- قسم عرض الكاش والبنك بالأسفل
----------------------------------------------------
local MoneyInfoContainer = Instance.new("Frame")
MoneyInfoContainer.Parent = MainFrame
MoneyInfoContainer.Size = UDim2.new(1, -16, 0, 42)
MoneyInfoContainer.Position = UDim2.new(0, 8, 1, -48)
MoneyInfoContainer.BackgroundColor3 = Color3.fromRGB(25, 25, 32)
MoneyInfoContainer.BackgroundTransparency = 0.3
MoneyInfoContainer.ZIndex = 52

local MoneyInfoCorner = Instance.new("UICorner")
MoneyInfoCorner.CornerRadius = UDim.new(0, 8)
MoneyInfoCorner.Parent = MoneyInfoContainer

local CashTextLabel = Instance.new("TextLabel")
CashTextLabel.Parent = MoneyInfoContainer
CashTextLabel.Size = UDim2.new(0.5, -5, 1, 0)
CashTextLabel.Position = UDim2.new(0, 8, 0, 0)
CashTextLabel.BackgroundTransparency = 1
CashTextLabel.Text = "Cash: 0"
CashTextLabel.TextColor3 = Color3.fromRGB(220, 220, 100)
CashTextLabel.TextSize = 12
CashTextLabel.Font = Enum.Font.GothamBold
CashTextLabel.TextXAlignment = Enum.TextXAlignment.Left
CashTextLabel.ZIndex = 53

local AtmTextLabel = Instance.new("TextLabel")
AtmTextLabel.Parent = MoneyInfoContainer
AtmTextLabel.Size = UDim2.new(0.5, -5, 1, 0)
AtmTextLabel.Position = UDim2.new(0.5, 0, 0, 0)
AtmTextLabel.BackgroundTransparency = 1
AtmTextLabel.Text = "Bank: 0"
AtmTextLabel.TextColor3 = Color3.fromRGB(100, 220, 100)
AtmTextLabel.TextSize = 12
AtmTextLabel.Font = Enum.Font.GothamBold
AtmTextLabel.TextXAlignment = Enum.TextXAlignment.Left
AtmTextLabel.ZIndex = 53
...
ContentArea.Size = UDim2.new(1, -135, 1, -100)
...
local waypoints = {
    ...
    Vector3.new(146.79, 255.47, 204.95),
            ...
            while autoFarm do
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if not isInsideGrocery(hrp.Position) then
                        ...
                        if distToPoint1 <= 60 then
                            for i = 1, 3 do
                                if not autoFarm then break end
                                moveToPosition(hrp, waypoints[i], speed)
                            end
                            for i = 4, #waypoints - 2 do
                                if not autoFarm then break end
                                moveToPosition(hrp, waypoints[i], speed)
                            end
                        else
...
----------------------------------------------------
-- جلب وتحديث الكاش والبنك تلقائياً
----------------------------------------------------
task.spawn(function()
    while true do
        pcall(function()
            ...
            if LocalPlayer:GetAttribute("Cash") ~= nil then cashVal = LocalPlayer:GetAttribute("Cash") end
            if LocalPlayer:GetAttribute("Bank") ~= nil then bankVal = LocalPlayer:GetAttribute("Bank") end
            if LocalPlayer:GetAttribute("Money") ~= nil and not cashVal then cashVal = LocalPlayer:GetAttribute("Money") end

            local searchFolders = {"leaderstats", "Data", "PlayerData", "Stats", "values", "Information"}
            for _, folderName in ipairs(searchFolders) do
                local folder = LocalPlayer:FindFirstChild(folderName)
                if folder then
                    for _, child in ipairs(folder:GetChildren()) do
                        local cName = string.lower(child.Name)
                        if (cName == "cash" or cName == "money" or cName == "wallet") and not cashVal then
                            cashVal = child.Value
                        elseif (cName == "bank" or cName == "balance" or cName == "bankbalance") and not bankVal then
                            bankVal = child.Value
                        end
                    end
                end
            ...
            if not cashVal or not bankVal then
                for _, obj in ipairs(LocalPlayer:GetDescendants()) do
                    if obj:IsA("ValueBase") then
                        local oName = string.lower(obj.Name)
                        if (oName == "cash" or oName == "money" or oName == "wallet") and not cashVal then
                            cashVal = obj.Value
                        elseif (oName == "bank" or oName == "balance") and not bankVal then
                            bankVal = obj.Value
                        end
                    end
                end
            ...
            if not cashVal or not bankVal then
                for _, gui in ipairs(PlayerGui:GetDescendants()) do
                    if gui:IsA("TextLabel") and gui.Visible then
                        ...
                        if (string.find(lowerName, "cash") or string.find(lowerTxt, "cash") or string.find(txt, "$")) and not cashVal then
                            local num = txt:gsub("[^%d]", "")
                            if num ~= "" then cashVal = num end
                        end
                        
                        if (string.find(lowerName, "bank") or string.find(lowerTxt, "bank")) and not bankVal then
                            local num = txt:gsub("[^%d]", "")
                            if num ~= "" then bankVal = num end
                        end
                    end
                end
            end

            CashTextLabel.Text = "Cash: " .. tostring(cashVal or 0)
            AtmTextLabel.Text = "Bank: " .. tostring(bankVal or 0)
        end)
        task.wait(0.5)
    end
end)
