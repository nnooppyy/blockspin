            ...
            while autoFarm do
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if not isInsideGrocery(hrp.Position) then
                        -- التحقق: إذا كان اللاعب قريب من نقطة البداية (رقم 0) بأقل من 60 ستود
                        local distToStart = (hrp.Position - customStartPoint).Magnitude
                        if distToStart <= 60 then
                            -- يذهب إلى النقطة رقم 4 مباشرة
                            if not autoFarm then break end
                            moveToPosition(hrp, waypoints[4], speed)

                            -- ثم يكمل باقي طريقه من 5 إلى آخره
                            for i = 5, #waypoints - 2 do
                                if not autoFarm then break end
                                moveToPosition(hrp, waypoints[i], speed)
                            end
                        else
                            -- إذا لم يكن قريباً، يبدأ مباشرة من النقطة 4 وتوابعها
                            if not autoFarm then break end
                            moveToPosition(hrp, waypoints[4], speed)
