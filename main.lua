...
-- ✅ متغير ارتفاع الشخصية عن الأرض (غيّر الرقم كما تريد)
local heightOffset = 1.25 -- ربع طول الشخصية القياسي
...
-- ✅ متغيرات ووظائف اختراق الجدران (NoClip)
local noclipConnection1 = nil
local noclipConnection2 = nil

local function enableNoClip()
    noclipConnection1 = RunService.Stepped:Connect(function()
    ...
    noclipConnection2 = RunService.RenderStepped:Connect(function()
        if LocalPlayer.Character then
            for _, part in pairs(LocalPlayer.Character:GetDescendants()) do
                if part:IsA("BasePart") then
                    part.CanCollide = false
...
local function moveToPosition(hrp, targetPos, speed)
    ...
    -- ✅ إضافة الارتفاع المطلوب فوق الأرض
    local safePos = Vector3.new(targetPos.X, targetPos.Y + heightOffset, targetPos.Z)
            ...
            if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if playerHasBox() then
                    ...
                    if realDeliveryPoint then
                        ...
                        -- ✅ إضافة الارتفاع هنا أيضاً
                        moveToPosition(hrp, Vector3.new(realDeliveryPoint.X, realDeliveryPoint.Y, realDeliveryPoint.Z), speed)
                        if not autoFarm then break end
                        ...
                        -- المشي تدريجياً عبر النقاط العكسية (من 7 إلى 6) لضمان عدم حدوث تيليبرورت
                        moveToPosition(hrp, waypoints[7], speed)
                        if not autoFarm then break end
                        moveToPosition(hrp, waypoints[6], speed)
                    else
