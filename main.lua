...
-- رجعت لك كل النقاط من 1 إلى 8 بالترتيب الكامل
local waypoints = {
    [1] = Vector3.new(61.82, 254.96, 56.74),
    [2] = Vector3.new(47.26, 254.76, 263.28),
    [3] = Vector3.new(160.40, 254.86, 250.37),
    [4] = Vector3.new(160.37, 255.18, 227.71),
    [5] = Vector3.new(159.49, 254.99, 204.32),
    [6] = Vector3.new(164.69, 254.99, 202.29), -- التوظيف
    [7] = Vector3.new(148.91, 254.74, 208.63), -- الصناديق
    [8] = Vector3.new(125.54, 254.74, 201.89), -- ممر الرفوف
}
...
local hasCompletedInitialPath = false -- متغيّر لضمان عدم العودة إلى 1 و2 و3 بعد الوصول للمحل
...
-- إلغاء الانحناء/الجلوس واستكمال الوقوف فوراً
local function cancelCrouchOrSit()
    ...
    if animator then
        for _, track in ipairs(animator:GetPlayingAnimationTracks()) do
            ...
            if string.find(animName, "sit") or string.find(animName, "crouch") or string.find(animName, "bend") or string.find(animName, "seat") then
                track:Stop(0)
            end
        end
    end
    ...
    for _, part in ipairs(char:GetDescendants()) do
        if part:IsA("SeatWeld") then
            part:Destroy()
        end
    end
end

-- حلقة فحص سريعة لتكسير الانحناء فوراً
task.spawn(function()
    while true do
        task.wait(0.01)
        if autoFarm then
...
local function toggleVisualPoints()
    ...
    if showPointsEnabled then
        ...
        for i = 1, 8 do
            createMarker(waypoints[i], BrickColor.new("Bright blue"), "WP " .. i)
        end
    ...
    task.spawn(function()
        while autoFarm do
            ...
            if localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                -- المرة الأولى فقط: يمر بالترتيب الكامل 1 -> 2 -> 3 -> 4 -> 5 -> 6 للتوظيف
                if not hasCompletedInitialPath then
                    for i = 1, 6 do
                    ...
                    if autoFarm then
                        ...
                        clickCenter()
                        task.wait(0.5)
                        hasCompletedInitialPath = true -- تم إكمال المسار الأولي ولن يعود للنقاط (1 و 2 و 3) مجدداً
                    end
                end

                -- الآن: الدورة التكرارية الثابتة داخل المحل بين 7 و 8
                ...
                if playerHasBox() then
                    ...
                    if realDeliveryPoint then
                        ...
                        task.wait(0.2)
                        -- يخرج للممر ويرجع مباشرة إلى نقطة 7 (الصناديق) بدون لمس 3
                        moveToPosition(hrp, targetAislePoint, speed)
                        task.wait(0.1)
                    else
                        task.wait(0.5)
