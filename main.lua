...
-- دالة للتحقق مما إذا كان اللاعب يمسك أداة حالياً ويقوم باستعمالها (مثل الصندوق)
local function isUsingBoxActive()
    local char = LocalPlayer.Character
    if not char then return false end
    for _, child in ipairs(char:GetChildren()) do
        if child:IsA("Tool") then
            -- إذا كانت الأداة مفعلة أو يتم تلويحها/استعمالها
            return true
        end
    end
    return false
                    ...
                    if not isDepositing then
                        ...
                        while autoFarm and playerHasBox() and not isDepositing do
                            ...
                            if realDeliveryPoint then
                                ...
                                -- التفتيش: هل هو في وسط الدائرة (وصل للنقطة) وهل الصندوق غير مستعمل؟ إذا لا، اضغط 2 لاستخدامه
                                task.wait(0.05)
