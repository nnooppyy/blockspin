...
TabOtherButton.Size = UDim2.new(0.9, 0, 0, 38)
        ...
        task.spawn(function()
            if not playerHasBox() then
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    -- المسار الجديد: إذا كان داخل البقالة يمشي للنقطة 8، ثم 7، ثم 6
                    ...
                    -- الضغط في منتصف الشاشة للتوظيف عند نقطة 6
                    if autoFarm then
                        task.wait(0.3)
                        clickCenter()
                        task.wait(0.5)
            ...
            while autoFarm do
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    -- عند أخذ الصندوق، الذهاب لنقطة 8 والتوصيل
...
----------------------------------------------------
-- 🔴 زر الرسبون الدقيق (يضغط ضغطة واحدة فقط عند الموت) 🔴
----------------------------------------------------
...
RespawnToggleContainer.MouseButton1Click:Connect(function()
    ...
    if autoRespawn then
        ts:Create(RespawnSwitchBackground, info, {BackgroundColor3 = Color3.fromRGB(180, 50, 50)}):Play()
...
task.spawn(function()
    local hasClickedForThisDeath = false
    
    while true do
        task.wait(0.3)
        if autoRespawn then
            local pGui = LocalPlayer:FindFirstChild("PlayerGui")
            local foundRespawnButton = false
            
            if pGui then
                for _, gui in ipairs(pGui:GetDescendants()) do
                    if gui:IsA("TextButton") and gui.Visible then
                        local text = string.lower(gui.Text or "")
                        -- يتحقق أن الزر يحتوي على كلمة respawn
                        if string.find(text, "respawn") then
                            foundRespawnButton = true
                            
                            if not hasClickedForThisDeath then
            ...
            -- إذا اختفى الزر (يعني اللاعب قام بالسلامة ورجع للعبة)، نعيد ضبط الحالة عشان يقدر يضغط بالموتة الجاية
            if not foundRespawnButton then
                hasClickedForThisDeath = false
            end
        end
    end
end)
