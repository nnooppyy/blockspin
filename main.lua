...
local waypoints = {
    ...
    Vector3.new(164.69, 254.99, 202.29), -- نقطة 7 (توظيف)
    Vector3.new(146.79, 255.47, 204.95),  -- نقطة 8 (صناديق)
...
local jobChecked = false
local boxesTaken = 0
local TARGET_BOXES = 6
...
-- دالة الضغط بدون ما تحتاج تقرب الشاشة
local function fireClosestPrompt(maxDistance)
    ...
    if closestPrompt then
        ...
        pcall(function()
            ...
            task.wait(closestPrompt.HoldDuration > 0 and closestPrompt.HoldDuration or 0.15)
...
-- فحص ووظيفة مرة واحدة فقط
local function checkAndApplyJob()
    if jobChecked then return end
    jobChecked = true
    ...
    for _, gui in ipairs(PlayerGui:GetDescendants()) do
        if gui:IsA("TextButton") or gui:IsA("TextLabel") then
            ...
            if string.find(text, "apply") then
                ...
                clickAtScreenPosition(absPos.X + absSize.X/2, absPos.Y + absSize.Y/2)
                task.wait(0.5)
                break
...
-- مسار الإيداع (مع الضغط بدون قرب الشاشة)
local function executeDepositSequence(speed)
    ...
    local success = fireClosestPrompt(12)
    if not success then
        pressEKey()
    end
    task.wait(0.6)
            ...
            while autoFarm do
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if not isDepositing then
                        -- فحص الوظيفة مرة واحدة فقط
                        if not jobChecked then
                        ...
                        -- أخذ الصناديق (حتى 6)
                        if boxesTaken < TARGET_BOXES and not playerHasBox() then
                            ...
                            for _ = 1, 6 do
                                ...
                                local success = fireClosestPrompt(14)
                                if not success then
                                    pressEKey()
                                end
                                task.wait(0.3)
                        ...
                        -- تسليم الصندوق
                        while autoFarm and playerHasBox() and not isDepositing and boxesTaken < TARGET_BOXES do
                        ...
                        if boxesTaken >= TARGET_BOXES then
                            boxesTaken = 0
                        end
                    end
                    task.wait(0.05)
