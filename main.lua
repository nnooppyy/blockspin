...
FarmContent.CanvasSize = UDim2.new(0, 0, 0, 180)
...
local depositCooldownTime = 0 -- متغير لحفظ وقت الحظر المؤقت في حال فشل الإيداع
...
-- التأكد أن اللاعب داخل البقالة قبل الإيداع
...
-- =========================================================================
-- مسار الإيداع مع نظام الفحص والرجوع في حال الفشل
-- =========================================================================
local function executeDepositSequence(speed)
    ...
    pressEKey()
    task.wait(0.5) -- انتظار الشاشة تطلع
    ...
    task.wait(0.5) -- انتظار لمعرفة هل تمت العملية بنجاح أم لا
    
    local cashAfterDeposit = getPlayerCash()

    -- يرجع למكان الرفوف بكل الأحوال
    moveToPositionHelper(hrp, aislePoints[3], speed)
    moveToPositionHelper(hrp, waypoints[9], speed)

    -- فحص هل نقصت الفلوس؟ (يعني الإيداع نجح)
    if cashAfterDeposit >= cashBeforeDeposit then
        -- فشل الإيداع (الشاشة ما طلعت، أو ما استجاب)
        depositCooldownTime = tick() + 25 -- حظر الإيداع لـ 25 ثانية القادمة
        hasDepositedThisCycle = false -- حتى يقدر يحاول بعدين
    else
        -- الإيداع نجح
        ...
        task.spawn(function()
            while autoFarm do
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    -- شرط الإيداع الجديد: 
                    -- 1. مبلغ كافي
                    -- 2. لم يودع هذه الدورة
                    -- 3. اللاعب داخل حدود البقالة حصراً
                    -- 4. وقت الحظر المؤقت 25 ثانية انتهى (إن وجد)
                    if autoDepositEnabled and (currentCash >= currentTargetVal) and not hasDepositedThisCycle and isInsideGrocery(hrp.Position) and not isDepositing then
                        if tick() >= depositCooldownTime then
                            isDepositing = true
                    ...
                    if currentCash < currentTargetVal then
                    ...
                    if not isDepositing then
                            ...
                            if distToPreStart <= proximityThreshold or distToPoint0 <= proximityThreshold then
                                ...
                                if autoFarm then
                                    for i = 4, targetEndIndex do
                                        -- إذا تفعل الإيداع وانتهى الحظر، وقف الحركة للرفوف وروح أودع
                                        if not autoFarm or (autoDepositEnabled and getPlayerCash() >= (tonumber(DepositInputBox.Text) or targetDepositAmount) and not hasDepositedThisCycle and tick() >= depositCooldownTime) then break end
                        ...
                        if not playerHasBox() then
                            ...
                            for _ = 1, 2 do
                                if not autoFarm or playerHasBox() or (autoDepositEnabled and getPlayerCash() >= (tonumber(DepositInputBox.Text) or targetDepositAmount) and not hasDepositedThisCycle and tick() >= depositCooldownTime) then break end
                                pressEKey()
                                task.wait(0.05)
                            end
                            task.wait(0.1)
                        ...
                        while autoFarm and playerHasBox() and not isDepositing do
                            if autoDepositEnabled and getPlayerCash() >= (tonumber(DepositInputBox.Text) or targetDepositAmount) and not hasDepositedThisCycle and tick() >= depositCooldownTime then break end
                            ...
                            if realDeliveryPoint then
                                ...
                                -- متغير العداد الزمني لفحص الصندوق كل ثانية
                                local lastBoxCheckTime = tick()

                                -- حلقة التعبئة (داخل الدائرة)
                                while autoFarm and isTargetStillActive(realDeliveryPoint) and not isDepositing do
                                    ...
                                    -- 1. فحص الخروج من الدائرة (بسبب التبقيس) ويرجعه
                                    if distFromCenter > 3.5 then
                                        hrp.CFrame = CFrame.new(currentPos)
                                        task.wait(0.1)
                                    end
                                    
                                    -- 2. فحص هل الصندوق بيده كل ثانية (فقط وهو جوا الدائرة)
                                    if tick() - lastBoxCheckTime >= 1 then
                                        lastBoxCheckTime = tick()
                                        if not isUsingBoxActive() then
                                            pressTwoKey()
                                        end
                                    end
