...
----------------------------------------------------
-- 3. محتوى قسم الأخرى (Other) مع ضبط Auto Respawn الشرطي الدقيق
----------------------------------------------------
...
----------------------------------------------------
-- منطق الـ Auto Respawn (التحقق الشرطي الدقيق: لا يضغط إلا بوجود الكلمة)
----------------------------------------------------
...
task.spawn(function()
    ...
    while true do
        task.wait(0.2) -- فحص مستمر هادئ وسريع لمعرفة حالة الشاشة
        if autoRespawnEnabled then
            local targetElement = nil
            
            -- البحث عن عنصر يحمل اسم أو نص "respawn" وظاهر على الشاشة
            ...
            -- إذا وجدت الكلمة والزر ظاهر، اضغط عليه حتى تختفي الكلمة تماماً
            if targetElement and targetElement.Parent and targetElement.Visible then
                while targetElement and targetElement.Parent and targetElement.Visible and autoRespawnEnabled do
                    ...
                    task.wait(0.3) -- سرعة الضغط أثناء ظهور شاشة الموت فقط
                    
                    -- إعادة التحقق هل اختفت الكلمة أم لا
                    local stillExists = false
                    if targetElement and targetElement.Parent and targetElement.Visible then
                        local name = string.lower(targetElement.Name or "")
                        local text = ""
                        if targetElement:IsA("TextButton") or targetElement:IsA("TextLabel") then
                            text = string.lower(targetElement.Text or "")
                        end
                        if string.find(name, "respawn") or string.find(text, "respawn") then
                            stillExists = true
                        end
...
----------------------------------------------------
-- المنطق والإحداثيات (كود الفارم الأصلي)
----------------------------------------------------
local waypoints = {
