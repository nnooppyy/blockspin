...
-- زر الدائرة العائم المستقل (لإظهار وإخفاء القائمة)
...
FloatingToggle.BackgroundColor3 = Color3.fromRGB(50, 200, 50)
FloatingToggle.Text = "GUI"
...
-- قفل حركة الكاميرا والتحكم باللعبة أثناء التفريم
local function lockGameInput(lock)
    if lock then
        -- قفل الكاميرا
        ContextActionService:BindAction("DisableCameraMovement", function() return Enum.ContextActionResult.Sink end, false, Enum.UserInputType.MouseMovement, Enum.UserInputType.Touch)
    else
        -- فك قفل الكاميرا
...
FloatingToggle.MouseButton1Click:Connect(function()
    ...
    if guiVisible then
        ...
        FloatingToggle.BackgroundColor3 = Color3.fromRGB(200, 50, 50)
    end
end)
...
FarmButton.MouseButton1Click:Connect(function()
    ...
    if autoFarm then
        ...
        lockGameInput(true) -- قفل الكاميرا والتحكم باللعبة
    else
        FarmButton.Text = "فرام صناديق: [معطل]"
        FarmButton.BackgroundColor3 = Color3.fromRGB(50, 150, 50)
        VirtualInputManager:SendKeyEvent(false, Enum.KeyCode.LeftShift, false, game)
        lockGameInput(false) -- فك قفل الكاميرا
    end
