...
----------------------------------------------------
-- منطق الـ Auto Respawn المعدل بالكامل (إيقاف فور الاختفاء)
----------------------------------------------------
...
-- دالة تتحقق من رؤية الزر وجميع واجهاته الأب بالكامل
    ...
    while true do
        ...
        if autoRespawnEnabled then
            pcall(function()
                ...
                -- يضغط طالما الزر لا يزال مرئياً بوضوح، يقطع مباشرة أول ما يختفي
                while autoRespawnEnabled and targetButton and isTrulyVisible(targetButton) do
                    ...
                    task.wait(0.15) -- انتظار قصير للتحقق من الاختفاء في الدورة القادمة فوراً
                end
            end)
        end
    end
...
local function moveToPosition(hrp, targetPos, speed)
    ...
    until tick() >= expectedEnd or not autoFarm or not LocalPlayer.Character or not LocalPlayer.Character:FindFirstChild("Humanoid") or LocalPlayer.Character.Humanoid.Health <= 0
    
    if not autoFarm or not LocalPlayer.Character or not LocalPlayer.Character:FindFirstChild("Humanoid") or LocalPlayer.Character.Humanoid.Health <= 0 then 
        tween:Cancel() 
    end
end
...
FarmToggleContainer.MouseButton1Click:Connect(function()
    ...
    if autoFarm then
        ...
        -- اللوب الأساسي المستمر للتجميع والعودة عند الموت
        task.spawn(function()
            while autoFarm do
                local char = LocalPlayer.Character
                ...
                if hrp and humanoid and humanoid.Health > 0 then
                    ...
                    -- 1. التأكد هل اللاعب خارج البقالة (سواء توه مفعل أو مات ورسبن خارج البقالة)
                    if not isInsideGrocery(hrp.Position) then
                        local startIndex = getClosestStartPoint(hrp.Position)
                        for i = startIndex, 3 do
                            if not autoFarm or humanoid.Health <= 0 then break end
                        ...
                        -- كليك بالمنتصف لبدء الوظيفة/الشفت إذا كان يحتاج
                        if autoFarm and humanoid.Health > 0 then
                            task.wait(0.3)
                            clickCenter()
                            task.wait(0.5)
                        end
                    ...
                    -- 2. إذا أصبح اللاعب داخل البقالة وعايش، يكمل دورة الفارم
                    if autoFarm and isInsideGrocery(hrp.Position) and humanoid.Health > 0 then
                        -- أخذ صندوق إذا ما عنده
                        if not playerHasBox() then
                            moveToPosition(hrp, waypoints[7], speed)
                            if autoFarm and humanoid.Health > 0 then
                                task.wait(0.3)
                                for _ = 1, 8 do
                                    if not autoFarm or playerHasBox() or humanoid.Health <= 0 then break end
                                    pressEKey()
                                    task.wait(0.2)
                                end
                                task.wait(0.3)
                        ...
                        -- توصيل الصندوق إذا كان معه
                        if autoFarm and playerHasBox() and humanoid.Health > 0 then
                            ...
                            if autoFarm and humanoid.Health > 0 then
                                ...
                                if realDeliveryPoint then
                                    ...
                                    if autoFarm and humanoid.Health > 0 then
                                        ...
                                        while autoFarm and humanoid.Health > 0 and isTargetStillActive(realDeliveryPoint) do
                                            checkAndUnsit(humanoid)
                                            task.wait(0.3)
                                        end
