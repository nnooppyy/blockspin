...
-- تم زيادة الحجم لاستيعاب الزر الجديد والمربع
...
DepositToggleContainer.Size = UDim2.new(0.95, 0, 0, 40)
...
DepositInputBox.Size = UDim2.new(0.95, 0, 0, 35)
...
----------------------------------------------------
local preStartPoint = Vector3.new(-96.79, 254.37, 555.12)  -- النقطة التي يذهب إليها قبل صفر
local customStartPoint = Vector3.new(-67.63, 254.62, 550.38) -- نقطة 0
local autoDepositPoint = Vector3.new(94.06, 254.74, 167.08) -- إحداثية الإيداع الجديدة
...
local aislePoints = {
    Vector3.new(121.61, 255.32, 202.77), -- رف 1
    Vector3.new(125.51, 255.32, 184.90), -- رف 2
...
ShowPointsButton.MouseButton1Click:Connect(function()
    ...
    if pointsVisible then
        ...
        -- إظهار نقطة Pre-Start
        local prePart = Instance.new("Part")
        prePart.Size = Vector3.new(2, 2, 2)
        prePart.Position = preStartPoint
        prePart.Anchored = true
        prePart.CanCollide = false
        prePart.Transparency = 0.5
        prePart.BrickColor = BrickColor.new("Magenta")
        prePart.Parent = pointsFolder
        
        local preBb = Instance.new("BillboardGui")
        preBb.Size = UDim2.new(0, 100, 0, 50)
        preBb.StudsOffset = Vector3.new(0, 3, 0)
        preBb.AlwaysOnTop = true
        preBb.Parent = prePart
        
        local preLbl = Instance.new("TextLabel")
        preLbl.Size = UDim2.new(1, 0, 1, 0)
        preLbl.BackgroundTransparency = 1
        preLbl.Text = "Pre-Start Point"
        preLbl.TextColor3 = Color3.fromRGB(255, 0, 255)
        preLbl.TextSize = 16
        preLbl.Font = Enum.Font.GothamBold
        preLbl.TextStrokeTransparency = 0
        preLbl.Parent = preBb
...
----------------------------------------------------
-- متغيرات ووظائف الإيداع التلقائي (الجديدة)
----------------------------------------------------
...
----------------------------------------------------
-- منطق التنقل والحركة (Farm Logic - المعدل)
----------------------------------------------------
        ...
        task.spawn(function()
            while autoFarm do
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    -- ==========================================
                    -- منطق الإيداع (يتم التحقق منه هنا)
                    -- ==========================================
                    if autoDepositEnabled and getPlayerCash() >= targetDepositAmount then
                        if isInsideGrocery(hrp.Position) then
                            -- التوجه لنقطة 8 أولاً
                            moveToPosition(hrp, waypoints[8], speed)
                            if not autoFarm then break end
                            
                            -- التوجه لرف 3 الداخلي
                            moveToPosition(hrp, aislePoints[3], speed)
                            if not autoFarm then break end
                            
                            -- التوجه لإحداثية الإيداع
                            moveToPosition(hrp, autoDepositPoint, speed)
                            if not autoFarm then break end
                            
                            task.wait(0.3)
                            
                            -- ضغط E بشكل متكرر لعمل الإيداع
                            for _ = 1, 10 do
                                pressEKey()
                                task.wait(0.1)
                            end
                            ...
                            -- إعادة بدء دورة الفارم من جديد ليتعرف على مكانه
                            continue
                        end
                    end
                    -- ==========================================
                    ...
                    if not isInsideGrocery(hrp.Position) then
                        ...
                        if distToPreStart <= proximityThreshold or distToPoint0 <= proximityThreshold then
                            -- الذهاب للنقطة الجديدة أولاً ثم نقطة صفر ثم مباشرة لنقطة 4
                            moveToPosition(hrp, preStartPoint, speed)
                            if autoFarm then
                        ...
                        elseif distToPoint1 <= proximityThreshold then
                            -- قريب من نقطة 1: الذهاب تسلسلياً (1 -> 2 -> 3 -> 4) وإكمال باقي المسار
