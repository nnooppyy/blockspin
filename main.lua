...
-- دالة التحقق من حدود البقالة بناءً على الإحداثيات التي أرسلتها
local function isInsideGrocery(pos)
    local minX, maxX = 85, 172
    local minZ, Zmax = 158, 218
    return pos.X >= minX and pos.X <= maxX and pos.Z >= minZ and pos.Z <= Zmax
end
...
local function findHiddenDeliveryTarget()
    ...
    if not char or not char:FindFirstChild("HumanoidRootPart") then nil end
            ...
            if localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if not playerHasBox() then
                    ...
                    if isInsideGrocery(hrp.Position) then
                        -- إذا كان داخله بالفعل، يتجه مباشرة لصناديق (نقطة 7) دون العودة لنقطة 3
                        moveToPosition(hrp, waypoints[7], speed)
                    else
                        -- إذا كان خارجه، يبدأ التسلسل الطبيعي: من أقرب نقطة إلى 3، ثم 4 إلى 6
                        ...
                        -- عند النقطة 6: التأكد من حالة التوظيف
                        if autoFarm then
