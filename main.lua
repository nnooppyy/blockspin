...
-- الدالة الصارمة للتحقق من شروط الإيداع (تمنع تفعيل الإيداع إلا داخل البقالة)
local function checkDepositCondition(hrp)
    ...
    -- الشرط الأساسي: لازم يكون داخل حدود البقالة
...
-- تم تعديل الدالة للسماح بالحركة للإيداع حتى لو الفارم طافي
...
-- حلقة مراقبة مستقلة للإيداع (تعمل حتى لو الفارم طافي)
task.spawn(function()
    while true do
        task.wait(0.5)
        if autoDepositEnabled then
            ...
            if hrp and hum and hum.Health > 0 then
                ...
                -- إذا الفارم طافي وهو مو قاعد يودع، يفحص شروط الإيداع (لازم يكون داخل البقالة)
        ...
        task.spawn(function()
            while autoFarm do
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    -- استدعاء دالة الفحص الصارمة للإيداع
                            ...
                            if distToPreStart <= proximityThreshold or distToPoint0 <= proximityThreshold then
                                ...
                                if autoFarm then
                                    for i = 4, targetEndIndex do
                                        -- الخروج من اللوب فقط إذا تحقق شرط الإيداع وأنت داخل البقالة
