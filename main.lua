...
-- تم إضافة تحقق لموت اللاعب لإلغاء التوين فوراً
local function moveToPosition(hrp, targetPos, speed)
    ...
    repeat
        ...
        -- إلغاء فوري لو اللاعب مات أثناء المشي
        if not currentHum or currentHum.Health <= 0 then
            tween:Cancel()
            return
        end
        
        checkAndUnsit(currentHum)
        
        if isTeleportDetected() then
            tween:Pause()
            while isTeleportDetected() and autoFarm do task.wait(0.2) end
            if autoFarm and currentHum.Health > 0 then
    ...
    if autoFarm then
        ...
        task.spawn(function()
            while autoFarm do
                ...
                -- تم دمج التحقق من صحة اللاعب لتجنب التعليق بعد الموت
                if character and character:FindFirstChild("HumanoidRootPart") and character:FindFirstChild("Humanoid") and character.Humanoid.Health > 0 then
                    ...
                    -- إذا رسبن اللاعب بالخارج بعد الموت، سيكتشف ذلك ويعود للبقالة من جديد
                    if not isInsideGrocery(hrp.Position) then
                        ...
                        task.wait(0.2)
                        continue -- إعادة تقييم اللوب لضمان الدخول قبل أخذ الكراتين
                    end

                    if not playerHasBox() then
                        moveToPosition(hrp, waypoints[7], speed)
                        if not autoFarm or humanoid.Health <= 0 then continue end
                        task.wait(0.3)
                        
                        for _ = 1, 8 do
                            if not autoFarm or humanoid.Health <= 0 or playerHasBox() then break end
                            pressEKey()
                            task.wait(0.2)
                        end
                        task.wait(0.3)
                    ...
                    -- إذا كان اللاعب ميت أو لا تزال شخصيته قيد التحميل، انتظر ثم حاول مرة أخرى
                    task.wait(1)
                end
            end
        end)
