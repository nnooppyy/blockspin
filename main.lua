            ...
            while autoFarm do
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if not isInsideGrocery(hrp.Position) then
                        -- إذا كان اللاعب قريب من النقطة الأولى (أقل من 60 ستود مثلاً)، يبدأ من 1 ويمشي (1، 2، 3، 4)
                        -- وإذا لم يكن قريباً منها، يتخطاها ويبدأ من النقطة الرابعة مباشرة (أو حسب الحاجة بدون أي زيادة)
                        local distToFirst = (hrp.Position - waypoints[1]).Magnitude
                        
                        if distToFirst <= 60 then
                            for i = 1, 4 do
