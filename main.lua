...
local waypoints = {
    ...
    Vector3.new(148.91, 254.74, 208.63), -- النقطة 6
    Vector3.new(125.54, 254.74, 201.89)  -- النقطة 7
}

-- نقاط الممرات (تمت إضافة الإحداثيات الجديدة للنقطة الثالثة)
local aislePoints = {
    Vector3.new(121.61, 255.32, 202.77), -- النقطة 1
    Vector3.new(125.51, 255.32, 184.90), -- النقطة 3 (المحدثة)
...
-- نظام البحث الذكي للرف المطلوب عند تسليم الصناديق
...
FarmButton.MouseButton1Click:Connect(function()
    ...
    if autoFarm then
        ...
        -- الضغط المستمر على زر Shift (الركض) عند التشغيل
        VirtualInputManager:SendKeyEvent(true, Enum.KeyCode.LeftShift, false, game)
    else
        FarmButton.Text = "فرام صناديق: [معطل]"
        FarmButton.BackgroundColor3 = Color3.fromRGB(50, 150, 50)
        -- رفع الضغط عن زر Shift عند التعطيل
    ...
    task.spawn(function()
            ...
            if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if playerHasBox() then
                    ...
                    if realDeliveryPoint then
                        ...
                        -- 1. التوجه لنقطة الممر أولاً (مثال: النقطة الثالثة)
                        moveToPosition(hrp, targetAislePoint, speed)
                        if not autoFarm then break end
                        task.wait(0.2)
                        
                        -- 2. الذهاب للرف المراد تعبئته باستخدام البحث الذكي
                        ...
                        -- 3. الرجوع لنفس نقطة الممر التي انطلق منها أولاً
                        moveToPosition(hrp, targetAislePoint, speed)
                        task.wait(0.3)
                        
                        -- 4. العودة إلى النقطة 7 لإكمال الدورة
                        moveToPosition(hrp, waypoints[7], speed)
                        task.wait(0.3)
                    else
                        task.wait(1)
        ...
        -- ضمان إيقاف زر Shift عند انتهاء الفرام
        pcall(function()
            VirtualInputManager:SendKeyEvent(false, Enum.KeyCode.LeftShift, false, game)
        end)
    end)
