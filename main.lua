...
----------------------------------------------------
-- المنطق والإحداثيات (الفارم مع العودة التلقائية)
----------------------------------------------------
local waypoints = {
...
-- تعديل دالة الحركة لتنقطع فور موت اللاعب
local function moveToPosition(hrp, targetPos, speed)
    ...
    while isTeleportDetected() and autoFarm and humanoid.Health > 0 do task.wait(0.2) end
    ...
    until tick() >= expectedEnd or not autoFarm or humanoid.Health <= 0
    
    if not autoFarm or humanoid.Health <= 0 then tween:Cancel() end
end
...
FarmToggleContainer.MouseButton1Click:Connect(function()
    ...
    if autoFarm then
        ...
        -- اللوب الأساسي للفارم والرجوع التلقائي عند الموت
        task.spawn(function()
            while autoFarm do
                local char = LocalPlayer.Character
                if char then
                    ...
                    if hrp and hum and hum.Health > 0 then
                        ...
                        if not isInsideGrocery(hrp.Position) then
                            ...
                            -- ينقر بالمنتصف (لبدء الشفت لو كان يحتاج)
                            if autoFarm and hum.Health > 0 then
                                task.wait(0.3)
                                clickCenter()
                                task.wait(0.5)
                            end
                        ...
                        -- 2. إذا اللاعب داخل البقالة، يكمل الفارم الطبيعي
                        if isInsideGrocery(hrp.Position) and hum.Health > 0 then
                            checkAndUnsit(hum)
                            
                            -- الذهاب لجلب صندوق إذا لم يكن لديه واحد
                            if not playerHasBox() then
                                moveToPosition(hrp, waypoints[7], speed)
                                if autoFarm and hum.Health > 0 then
                                    task.wait(0.3)
                                    for _ = 1, 8 do
                                        if not autoFarm or playerHasBox() or hum.Health <= 0 then break end
                                        pressEKey()
                                        task.wait(0.2)
                                    end
                                    task.wait(0.3)
                            ...
                            -- إيصال الصندوق إذا كان يملكه
                            if autoFarm and playerHasBox() and hum.Health > 0 then
                                ...
                                if autoFarm and hum.Health > 0 then
                                    ...
                                    if realDeliveryPoint then
                                        ...
                                        while autoFarm and hum.Health > 0 and isTargetStillActive(realDeliveryPoint) do
                                            checkAndUnsit(hum)
                                            task.wait(0.3)
                                        end
                ...
                -- تأخير بسيط جداً عشان ما يصير لاق، ويعطي فرصة للرسباون
                task.wait(0.5)
            end
        end)
    end
