...
local function moveToPosition(hrp, targetPos, speed)
    ...
    repeat
        task.wait(0.05)
        -- شرط إيقاف إذا مات اللاعب وتدمر المجسم أثناء الحركة
        ...
        if isTeleportDetected() then
            tween:Pause()
            while isTeleportDetected() and autoFarm do task.wait(0.2) end
            if autoFarm and hrp and hrp.Parent and humanoid and humanoid.Health > 0 then
    ...
    if not autoFarm or (humanoid and humanoid.Health <= 0) then pcall(function() tween:Cancel() end) end
end
...
local function playerHasBox()
    if LocalPlayer.Character then
        ...
        -- تم التعديل لجلب الـ Backpack ديناميكياً لتفادي الخطأ عند موت اللاعب
    ...
    if autoFarm then
        ...
        task.spawn(function()
            if not playerHasBox() then
                -- تم إضافة شرط للتحقق من أن اللاعب على قيد الحياة
            ...
            while autoFarm do
                ...
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") and LocalPlayer.Character:FindFirstChild("Humanoid") and LocalPlayer.Character.Humanoid.Health > 0 then
                    ...
                    while autoFarm and playerHasBox() and humanoid.Health > 0 do
                        ...
                        if realDeliveryPoint then
                            ...
                            while autoFarm and isTargetStillActive(realDeliveryPoint) and humanoid.Health > 0 do
                                checkAndUnsit(humanoid)
                                task.wait(0.3)
                            end
