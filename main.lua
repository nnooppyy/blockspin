            ...
            if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if not playerHasBox() then
                    if isInsideGrocery(hrp.Position) then
                        -- إذا كان داخل البقالة، يتجه لأقرب نقطة داخلية (1 أو 2 أو 3) ثم للنقطة 7 والصناديق
                        local internalPoint = getClosestInternalAisle(hrp.Position)
                        moveToPosition(hrp, internalPoint, speed)
                        if not autoFarm then break end
                        
                        -- تمرير نقطة التوظيف وفحصها لضمان أنه موظف
                        ...
                        -- إذا كان بالخارج، يتبع التسلسل الطبيعي كاملاً
