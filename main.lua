...
-- العنوان مع رابط الديسكورد فوق يسار
...
-- نظام إظهار النقاط (Points) في اللعبة وإصلاحها
local pointsVisible = false
local pointsFolder = Instance.new("Folder")
pointsFolder.Name = "WaypointsVisualizer"
pointsFolder.Parent = workspace

local function updatePointsVisibility()
    pointsFolder:ClearAllChildren()
    if pointsVisible then
        -- دمج جميع النقاط لعرضها
        local allPointsToDraw = {
            ...
            Vector3.new(-299.67, 254.63, 299.28),
            Vector3.new(-72.29, 254.53, 300.58),
            Vector3.new(-304.36, 254.54, -396.38),
            Vector3.new(-68.26, 254.62, -211.59)
        }
        for _, pos in ipairs(allPointsToDraw) do
...
----------------------------------------------------
-- نظام التحقق من التوظيف (apply / Leave)
----------------------------------------------------
...
----------------------------------------------------
-- منطق الـ Auto Respawn المعدل بالكامل
----------------------------------------------------
...
----------------------------------------------------
-- المنطق والإحداثيات (الفارم مع إضافة النقاط الأربع الجديدة واختيار أقرب نقطة للبداية)
----------------------------------------------------
local waypoints = {
    Vector3.new(61.82, 254.96, 56.74),
    Vector3.new(47.26, 254.76, 263.28),
...
-- الإحداثيات الأربعة الجديدة المضافة
local extraStartPoints = {
    Vector3.new(-299.67, 254.63, 299.28),
    Vector3.new(-72.29, 254.53, 300.58),
    Vector3.new(-304.36, 254.54, -396.38),
    Vector3.new(-68.26, 254.62, -211.59)
...
-- اختيار أقرب نقطة من النقاط الأربع الجديدة أولاً، ثم التوجه إلى أول 3 نقاط أساسية
local function getClosestStartPoint(playerPos)
    local closestPoint = extraStartPoints[1]
    local minDistance = (playerPos - extraStartPoints[1]).Magnitude
    
    for i = 2, #extraStartPoints do
        local dist = (playerPos - extraStartPoints[i]).Magnitude
        if dist < minDistance then
            minDistance = dist
            closestPoint = extraStartPoints[i]
        end
    end
    return closestPoint
end
            ...
            while autoFarm do
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if not isInsideGrocery(hrp.Position) then
                        -- المرور بأقرب نقطة من الأربع نقاط الجديدة أولاً
                        local closestExtra = getClosestStartPoint(hrp.Position)
                        moveToPosition(hrp, closestExtra, speed)
                        
                        -- ثم التوجه إلى أول 3 نقاط أساسية
                        for i = 1, 3 do
