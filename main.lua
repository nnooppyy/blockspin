...
local hasDepositedThisCycle = false -- متغير لمنع التكرار المستمر للإيداع
...
-- =========================================================================
-- مسار الإيداع المطلوب (يتم تنفيذه مرة واحدة عند تخطي الحد المطلوب)
-- =========================================================================
local function executeDepositSequence(speed)
    ...
    -- الضغط على الزر الأول لفتح القائمة
    clickAtScreenPosition(372, 162)
    task.wait(0.6)
    
    -- قراءة المبلغ المكتوب من خانة الإدخال أو المبلغ المحدد
    local depositVal = tonumber(DepositInputBox.Text) or targetDepositAmount
    local currentCash = getPlayerCash()
    
    -- كتابة المبلغ المختار في السكربت (أو الرصيد المتوفر إذا كان أقل)
    local amountToDeposit = math.min(currentCash, depositVal)
    if amountToDeposit <= 0 then
        amountToDeposit = depositVal
    end
    
    typeInTextBox(amountToDeposit)
    task.wait(0.5)
    
    -- الضغط على زر التأكيد للإيداع نهائياً
    clickAtScreenPosition(366, 196)
    task.wait(0.6)
    
    -- العودة من نفس الطريق المعاكس أو إكمال المسار الطبيعي
    moveToPositionHelper(hrp, aislePoints[3], speed)
    task.wait(0.1)
    moveToPositionHelper(hrp, waypoints[9], speed)
end
        ...
        task.spawn(function()
            while autoFarm do
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    -- فحص المبلغ: إذا وصل للحد ولم يقم بالإيداع في هذه الدورة، يتم تنفيذه مرة واحدة فقط
                    local currentCash = getPlayerCash()
                    ...
                    if autoDepositEnabled and (currentCash >= currentTargetVal) and not hasDepositedThisCycle and isInsideGrocery(hrp.Position) and not isDepositing then
                        ...
                        hasDepositedThisCycle = true -- قفل التكرار حتى تنخفض الأموال أو تبدأ دورة جديدة
                        task.wait(1)
                        isDepositing = false
                        continue
                    end

                    -- إذا انخفض المبلغ عن الحد المطلوب، يتم إعادة تفعيل إمكانية الإيداع للدورة القادمة
                    ...
                    if not isDepositing then
                            ...
                            if distToPreStart <= proximityThreshold or distToPoint0 <= proximityThreshold then
                                ...
                                if autoFarm then
                                    for i = 4, targetEndIndex do
                                        if not autoFarm or (autoDepositEnabled and getPlayerCash() >= (tonumber(DepositInputBox.Text) or targetDepositAmount) and not hasDepositedThisCycle) then break end
                        ...
                        if not playerHasBox() then
                            ...
                            for _ = 1, 8 do
                                if not autoFarm or playerHasBox() or (autoDepositEnabled and getPlayerCash() >= (tonumber(DepositInputBox.Text) or targetDepositAmount) and not hasDepositedThisCycle) then break end
                                pressEKey()
                                task.wait(0.2)
                            end
                            task.wait(0.3)
                        ...
                        while autoFarm and playerHasBox() and not isDepositing do
                            if autoDepositEnabled and getPlayerCash() >= (tonumber(DepositInputBox.Text) or targetDepositAmount) and not hasDepositedThisCycle then break end
