...
-- الإحداثيات العامة (بدون النقطة 8 المحذوفة، الالتزام التام بالمسار الأصلي)
local waypoints = {
...
-- دالة المشي الثابتة على المسار مع التوقف التام عند التيليبرورت بدون أي شطح
local function moveToPosition(hrp, targetPos, speed)
    ...
    -- لو التيليبرورت ظاهر قبل ما تتحرك، انتظر لين يختفي
    ...
    repeat
        ...
        -- لو ظهرت رسالة التيليبرورت أثناء الحركة، أوقف الحركة فوراً وانظر حتى تختفي تماماً ثم أكمل المسار بنفس السرعة وبدقة
        ...
        task.spawn(function()
            ...
            while autoFarm do
                ...
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    -- الذهاب لمكان الصناديق حصراً عبر المسار
                    ...
                    -- إذا معه صندوق، يمشي بالمسار حصراً (من 7 إلى 8 ثم الرفوف)
                    if playerHasBox() then
                        ...
                        if realDeliveryPoint then
                            ...
                            -- الرجوع حصراً عبر المسار من 8 إلى 7 بالترتيب الصحيح
                            moveToPosition(hrp, waypoints[8], speed)
                            if not autoFarm then break end
