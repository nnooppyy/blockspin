...
----------------------------------------------------
-- 3. محتوى قسم الأخرى (Other) مع زر Auto Respawn المعدل
----------------------------------------------------
...
----------------------------------------------------
-- منطق الـ Auto Respawn المعدل بدقة لزر Respawn
----------------------------------------------------
...
task.spawn(function()
    ...
    while true do
        ...
        if autoRespawnEnabled then
            ...
            -- البحث عن زر Respawn الفعلي في واجهات اللعبة
            for _, gui in ipairs(PlayerGui:GetDescendants()) do
                if (gui:IsA("TextButton") or gui:IsA("ImageButton")) and gui.Visible then
                    local text = string.lower(gui.Text or "")
                    if string.find(text, "respawn") or string.find(gui.Name:lower(), "respawn") then
                        targetButton = gui
                        break
                    end
                end
            ...
            -- طالما الزر موجود ومرئي، يتم الضغط في منتصفه بدقة كل 3 ثوانٍ
            while targetButton and targetButton.Parent and targetButton.Visible and autoRespawnEnabled do
                ...
                if targetButton and targetButton.Parent and targetButton.Visible then
                    local text = string.lower(targetButton.Text or "")
                    if string.find(text, "respawn") or string.find(targetButton.Name:lower(), "respawn") then
                        stillExists = true
                    end
                end
