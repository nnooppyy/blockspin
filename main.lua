...
Title.Text = "قائمة التحكم (التوصيل المباشر للرف)"
...
-- النقاط الست للممرات (بدايات ونهايات الممرات الثلاثة كممر عبور آمن)
local aislePoints = {
            ...
            if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if playerHasBox() then
                    ...
                    if realDeliveryPoint then
                        ...
                        -- ب) من نقطة الممر، يتحرك فوراً وبشكل مباشر إلى الرف الفعلي (مع مسافة أمان وقوفامه)
                        local direction = (realDeliveryPoint - hrp.Position)
                        direction = Vector3.new(direction.X, 0, direction.Z).Unit
                        local finalShelfPos = realDeliveryPoint
                        if direction.Magnitude > 0 then
                            finalShelfPos = realDeliveryPoint - (direction * 2.5) -- يوقف قدام الرف بالضبط بدون اختراق
                        end
