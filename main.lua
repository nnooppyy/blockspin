...
-- تم إزالة الـ Tween/Linear واستبداله بالحركة الطبيعية (CFrame مباشرة)
local function moveToPosition(hrp, targetPos, speed)
    ...
    local moveTime = distance / math.max(speed, 1)
    
    local startTime = tick()
    local startCFrame = hrp.CFrame
    local targetCFrame = CFrame.new(safePos)
    ...
    repeat
        ...
        local alpha = math.clamp(elapsed / moveTime, 0, 1)
        ...
        if isTeleportDetected() then
            ...
            startTime = startTime + (tick() - pauseStart)
        end
    until alpha >= 1 or not autoFarm
end
