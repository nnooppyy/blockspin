...
-- دالة لتحديد أقرب نقطة داخل البقالة (النقاط 7 و 8 مثلاً) بدون اللف والدوران، ثم الذهاب مباشرة للنقطة 6
            ...
            if not playerHasBox() then
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if isInsideGrocery(hrp.Position) then
                        -- 1. يروح لأقرب نقطة داخل البقالة (مثلاً 8 أو 7)
                        local startIdx = getClosestGroceryWaypointIndex(hrp.Position)
                        moveToPosition(hrp, waypoints[startIdx], speed)
                        
                        if not autoFarm then return end
                        task.wait(0.1)
                        
                        -- 2. يروح مباشرة إلى النقطة 6 عشان يتوظف
                        moveToPosition(hrp, waypoints[6], speed)
                    else
                        -- المسار الطبيعي لو كان برى البقالة
