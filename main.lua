...
FarmButton.MouseButton1Click:Connect(function()
    ...
    if autoFarm then
        ...
        -- **طريقة التثبيت الوهمي المستمر لزر الشفت في الخلفية**
        task.spawn(function()
            while autoFarm do
                pcall(function()
                    VirtualInputManager:SendKeyEvent(true, Enum.KeyCode.LeftShift, false, game)
                end)
                task.wait(0.5) -- يعيد إرسال ضغطة الشفت كل نصف ثانية لضمان بقائه مفَعلاً مهما لمست الشاشة
            end
            -- عند الإيقاف، يطلق زر الشفت
            pcall(function()
