...
MainFrame.Size = UDim2.new(0, 360, 0, 260) -- تم زيادة الطول قليلاً لاستيعاب الزر الجديد
...
FarmContent.CanvasSize = UDim2.new(0, 0, 0, 220)
...
-- زر جوال بقالة الجديد
local GroceryPhoneToggleContainer = Instance.new("TextButton")
GroceryPhoneToggleContainer.Parent = FarmContent
GroceryPhoneToggleContainer.Size = UDim2.new(0.95, 0, 0, 36)
GroceryPhoneToggleContainer.BackgroundColor3 = Color3.fromRGB(45, 45, 55)
GroceryPhoneToggleContainer.BackgroundTransparency = 0.3
GroceryPhoneToggleContainer.Text = ""
GroceryPhoneToggleContainer.AutoButtonColor = false
GroceryPhoneToggleContainer.ZIndex = 52

local GroceryPhoneCorner = Instance.new("UICorner")
GroceryPhoneCorner.CornerRadius = UDim.new(0, 6)
GroceryPhoneCorner.Parent = GroceryPhoneToggleContainer

local GroceryPhoneLabel = Instance.new("TextLabel")
GroceryPhoneLabel.Parent = GroceryPhoneToggleContainer
GroceryPhoneLabel.Size = UDim2.new(1, -55, 1, 0)
GroceryPhoneLabel.Position = UDim2.new(0, 10, 0, 0)
GroceryPhoneLabel.BackgroundTransparency = 1
GroceryPhoneLabel.Text = "جوال بقالة 📱"
GroceryPhoneLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
GroceryPhoneLabel.TextSize = 12
GroceryPhoneLabel.Font = Enum.Font.GothamBold
GroceryPhoneLabel.TextXAlignment = Enum.TextXAlignment.Left
GroceryPhoneLabel.ZIndex = 53

local GrocerySwitchBackground = Instance.new("Frame")
GrocerySwitchBackground.Parent = GroceryPhoneToggleContainer
GrocerySwitchBackground.Size = UDim2.new(0, 38, 0, 20)
GrocerySwitchBackground.Position = UDim2.new(1, -44, 0.5, -10)
GrocerySwitchBackground.BackgroundColor3 = Color3.fromRGB(80, 80, 95)
GrocerySwitchBackground.ZIndex = 53

local GrocerySwitchBgCorner = Instance.new("UICorner")
GrocerySwitchBgCorner.CornerRadius = UDim.new(1, 0)
GrocerySwitchBgCorner.Parent = GrocerySwitchBackground

local GrocerySwitchKnob = Instance.new("Frame")
GrocerySwitchKnob.Parent = GrocerySwitchBackground
GrocerySwitchKnob.Size = UDim2.new(0, 16, 0, 16)
GrocerySwitchKnob.Position = UDim2.new(0, 2, 0.5, -8)
GrocerySwitchKnob.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
GrocerySwitchKnob.ZIndex = 54

local GrocerySwitchKnobCorner = Instance.new("UICorner")
GrocerySwitchKnobCorner.CornerRadius = UDim.new(1, 0)
GrocerySwitchKnobCorner.Parent = GrocerySwitchKnob
...
local waypoints = {
    ...
    Vector3.new(164.69, 254.99, 202.29), -- نقطة 7 (للتوظيف/التأكد)
...
local groceryPhoneEnabled = false -- متغير جوال بقالة
...
-- تفعيل وتحديث زر جوال بقالة
local function updateGroceryPhoneVisual()
    local ts = game:GetService("TweenService")
    local info = TweenInfo.new(0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.Out)
    if groceryPhoneEnabled then
        ts:Create(GrocerySwitchBackground, info, {BackgroundColor3 = Color3.fromRGB(50, 180, 80)}):Play()
        ts:Create(GrocerySwitchKnob, info, {Position = UDim2.new(1, -18, 0.5, -8)}):Play()
    else
        ts:Create(GrocerySwitchBackground, info, {BackgroundColor3 = Color3.fromRGB(80, 80, 95)}):Play()
        ts:Create(GrocerySwitchKnob, info, {Position = UDim2.new(0, 2, 0.5, -8)}):Play()
    end
end

GroceryPhoneToggleContainer.MouseButton1Click:Connect(function()
    groceryPhoneEnabled = not groceryPhoneEnabled
    updateGroceryPhoneVisual()
end)
                    ...
                    if not isDepositing then
                        if not isInsideGrocery(hrp.Position) then
                            ...
                            if distToPreStart <= proximityThreshold or distToPoint0 <= proximityThreshold then
                                ...
                                if autoFarm then
                                    -- إذا كان "جوال بقالة" شغال، يبدأ من النقطة 7، وإلا يكمل العادي
                                    local startIndexLoop = groceryPhoneEnabled and 7 or 4
                                    for i = startIndexLoop, targetEndIndex do
                                        if not autoFarm or checkDepositCondition(hrp) then break end
                                        moveToPositionHelper(hrp, waypoints[i], speed)
                                    end
                                end
                                if autoFarm then task.wait(0.1) clickCenter() task.wait(0.2) end
                            elseif distToPoint1 <= proximityThreshold then
                                local startIndexLoop = groceryPhoneEnabled and 7 or 1
                                for i = startIndexLoop, targetEndIndex do
                                    if not autoFarm or checkDepositCondition(hrp) then break end
                                    moveToPositionHelper(hrp, waypoints[i], speed)
                                end
                        ...
                        while autoFarm and playerHasBox() and not isDepositing do
                            ...
                            if realDeliveryPoint then
                                ...
                                moveToPositionHelper(hrp, Vector3.new(realDeliveryPoint.X, hrp.Position.Y, realDeliveryPoint.Z), speed)
                                if not autoFarm or checkDepositCondition(hrp) then break end
                                ...
                                while autoFarm and isTargetStillActive(realDeliveryPoint) and not isDepositing do
                                    ...
                                    local currentPos = Vector3.new(realDeliveryPoint.X, hrp.Position.Y, realDeliveryPoint.Z)
                                    local distFromCenter = (hrp.Position - currentPos).Magnitude
