...
local function updatePointsVisibility()
    ...
    if pointsVisible then
        for idx, pos in ipairs(newCustomPoints) do
...
----------------------------------------------------
-- المنطق والإحداثيات (الفارم مع الإحداثيات الثلاثة الجديدة بالترتيب)
----------------------------------------------------
local waypoints = {
...
-- اختيار أقرب نقطة من الثلاث نقاط الجديدة للبدء منها ثم السير بترتيبها حتى نهايتها
local function getClosestCustomStartIndex(playerPos)
    local closestIdx = 1
    local minDistance = math.huge
    for i, pos in ipairs(newCustomPoints) do
...
local function findHiddenDeliveryTarget()
    ...
    local searchCenter = waypoints[6]
    local targetPos = nil
    local minDist = 120
            ...
            while autoFarm do
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if not isInsideGrocery(hrp.Position) then
                        -- المرور بالإحداثيات الثلاثة الجديدة بدءاً من الأقرب وحتى نهايتها بالترتيب
                        local startIndex = getClosestCustomStartIndex(hrp.Position)
                        for i = startIndex, #newCustomPoints do
                            if not autoFarm then break end
                            moveToPosition(hrp, newCustomPoints[i], speed)
                        end
                        
                        -- ثم إكمال باقي المسار الأساسي
                    ...
                    if not playerHasBox() then
                        moveToPosition(hrp, waypoints[5], speed)
                        if not autoFarm then break end
                        task.wait(0.3)
