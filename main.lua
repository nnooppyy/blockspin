...
local waypoints = {
    ...
    Vector3.new(125.54, 254.74, 201.89)  -- النقطة 7 (بداية البحث عن الدائرة الفعالة)
}
...
-- خوارزمية البحث المتقدمة لاكتشاف الدائرة النشطة المطلوبة للتسليم
local function findActiveDeliveryCircle()
    local player = game.Players.LocalPlayer
    local char = player.Character
    if not char or not char:FindFirstChild("HumanoidRootPart") then return nil end
    local hrp = char.HumanoidRootPart

    -- 1. البحث عن أي ParticleEmitter أو Attachment نشط ينشئ تأثير الدائرة المضيئة
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj:IsA("ParticleEmitter") or obj:IsA("Beam") or obj:IsA("PointLight") or obj:IsA("SurfaceLight") then
            if obj.Enabled then
                local parentPart = obj:FindFirstAncestorWhichIsA("BasePart")
                if parentPart and not parentPart:IsDescendantOf(char) then
                    local dist = (hrp.Position - parentPart.Position).Magnitude
                    if dist < 120 then -- في حدود السوبرماركت
    ...
    -- 2. البحث عن الأجزاء الأسطوانية الشكل (MeshPart / Cylinder) المفعلة
    for _, part in ipairs(workspace:GetDescendants()) do
        if part:IsA("BasePart") and not part:IsDescendantOf(char) then
            if part.Transparency < 1 and part.CanCollide == false then
                -- فحص حجم الدائرة الزرقاء (غالبًا تكون مسطحة على الأرض ارتفاعها قليل)
                if part.Size.Y < 2 and (part.Size.X > 2 or part.Size.Z > 2) then
    ...
    task.spawn(function()
        ...
        if player.Character and player.Character:FindFirstChild("HumanoidRootPart") and autoFarm then
            ...
            for i = startIndex, #waypoints do
                ...
                -- النقطة 7: البحث الديناميكي عن الدائرة النشطة والتوجّه إليها
                elseif i == 7 then
                    task.wait(0.4)
                    local targetCirclePos = findActiveDeliveryCircle()
                    
                    if targetCirclePos then
                        moveToPosition(hrp, targetCirclePos, speed)
                    end
                    
                    -- التفريغ داخل الدائرة
                    task.wait(0.3)
                    for _ = 1, 4 do
                        pressEKey()
                        task.wait(0.2)
