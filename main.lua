...
local function clickAtScreenPosition(x, y)
    pcall(function()
        ...
        task.wait(0.02) -- تم تقليل التأخير هنا لزيادة السرعة
...
----------------------------------------------------
-- منطق التنقل والحركة وإيداع الأموال (سريع جداً)
----------------------------------------------------
...
local function pressEKey()
    for i = 1, 2 do -- تقليل عدد التكرارات ليكون أسرع بكثير
...
-- =========================================================================
-- مسار الإيداع السريع جداً (تم إزالة الانتظار الطويل وجعله فوري)
-- =========================================================================
local function executeDepositSequence(speed)
    ...
    task.wait(0.1) -- تقليل وقت الانتظار عند الوصول
    if not autoFarm then return end
    
    -- التفاعل الفوري مع الصراف (ضغط E سريع)
    pressEKey()
    task.wait(0.2)
    
    -- الضغط السريع على الزر الأول لفتح القائمة
    clickAtScreenPosition(372, 162)
    task.wait(0.2)
    
    -- جلب الرصيد الحالي وكتابته فوراً
    ...
    -- الضغط على زر التأكيد للإيداع نهائياً وبسرعة
    clickAtScreenPosition(366, 196)
    task.wait(0.2)
    
    -- العودة فوراً بدون تأخيرات
    moveToPositionHelper(hrp, aislePoints[3], speed)
    moveToPositionHelper(hrp, waypoints[9], speed)
end
