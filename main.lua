...
Title.Text = "قائمة التحكم (مُحدثة)"
...
-- ضغط مفتاح E (مُحسن لضمان التفاعل)
local function pressEKey()
    VirtualInputManager:SendKeyEvent(true, Enum.KeyCode.E, false, game)
    task.wait(0.2) -- تعليقة خفيفة على الزر لضمان أن اللعبة تستجيب
...
-- 🎯 الخوارزمية الجبارة اللي بنيناها بناءً على الفحص اللي سويته
local function findHiddenDeliveryTarget()
    ...
    local targetPos = nil
    local minDist = 120 -- نطاق البحث داخل المتجر فقط
    ...
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj:IsA("BasePart") and not obj:IsDescendantOf(char) then
            ...
            if dist <= minDist then
                ...
                -- الهدف الأول: بناءً على الفحص، الدائرة هي MeshPart اسمها Base
                if obj.Name == "Base" and obj:IsA("MeshPart") then
                    -- نتأكد أنه مجسم مسطح (ارتفاعه قليل)
                    if obj.Size.Y < 2.5 then
                        isMatch = true
                    end
                end
                
                -- الهدف الثاني: أو منطقة رف مخفية اسمها Shelf
                if obj.Name == "Shelf" and obj.Transparency == 1 then
                    -- نتأكد لو كان فيها أي تأثير ضوئي أو بصري فعّال
    ...
    task.spawn(function()
        ...
        if player.Character and player.Character:FindFirstChild("HumanoidRootPart") and autoFarm then
            ...
            for i = startIndex, #waypoints do
                ...
                -- النقطة 7: التوجه للدائرة الحقيقية فقط (Base أو Shelf المخفية)
                elseif i == 7 then
                    task.wait(0.4)
