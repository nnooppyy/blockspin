...
-- دالة قيام مكررة وقوية للتعامل مع نظام الجلوس
...
-- حلقة فحص سريعة ومستمرة للجلوس كل 0.01 ثانية
task.spawn(function()
    while true do
        task.wait(0.01)
        if autoFarm then
...
local function bindSeatedEvent(char)
    local humanoid = char:WaitForChild("Humanoid", 5)
    if humanoid then
        humanoid.Seated:Connect(function(active)
            if active and autoFarm then
                forceUnsit()
            end
        end)
    end
...
localPlayer.CharacterAdded:Connect(bindSeatedEvent)
if localPlayer.Character then bindSeatedEvent(localPlayer.Character) end
        ...
        while autoFarm do
            ...
            if localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if not playerHasBox() then
                    -- إذا لم يكن متوظفاً فقط يمر بالخطوات الأولى للتوظيف
                    ...
                    -- يتوجه مباشرة إلى الصناديق (نقطة 7) لأخذ صندوق جديد
                ...
                if playerHasBox() then
                    ...
                    if realDeliveryPoint then
                        ...
                        -- ينتظر حتى تنتهي التعبئة مع القيام المستمر
                        ...
                        task.wait(0.2)
                        -- يخرج إلى الممر الأساسي ثم يرجع لنقطة الصناديق مباشرة
                        moveToPosition(hrp, targetAislePoint, speed)
                        task.wait(0.1)
                    else
                        task.wait(0.5)
