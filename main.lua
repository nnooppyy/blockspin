...
-- دالة للتحقق الذكي هل اللاعب متوظف حالياً أم لا
local function isAlreadyEmployed()
    local player = game.Players.LocalPlayer
    
    -- 1. فحص هل يوجد صندوق أو أداة وظيفة في الشخصية أو الحقيبة
    if player.Character and (player.Character:FindFirstChildOfClass("Tool") or player.Backpack:FindFirstChildOfClass("Tool")) then
        return true
    end
    
    -- 2. فحص واجهات المستخدم (GUI) عن شارات الوظيفة
    local playerGui = player:FindFirstChild("PlayerGui")
    if playerGui then
        for _, v in ipairs(playerGui:GetDescendants()) do
            if v:IsA("TextLabel") or v:IsA("TextButton") then
                local txt = tostring(v.Text):lower()
                if txt:find("employee") or txt:find("موظف") or txt:find("استقالة") or txt:find("quit") then
                    return true
                end
            end
        end
...
-- دالة للتحرك التلقائي نحو موقع معين
...
-- الذكاء الاصطناعي: البحث عن السهم وتحديد وجهة التعبئة المحددة
local function findArrowDestination()
    local player = game.Players.LocalPlayer
    local char = player.Character
    if not char then return nil end

    -- 1. البحث عن Beam (خط أو سهم ممتد من اللاعب إلى الهدف)
    ...
    -- 2. البحث عن أسهم أو مؤشرات اتجاه بصرية داخل Workspace أو شخصية اللاعب
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj.Name:lower():find("arrow") or obj.Name:lower():find("target") or obj.Name:lower():find("destination") or obj.Name:lower():find("pointer") then
    ...
    -- 3. البحث عن BillboardGui أو العلامات المضيئة التي تشير لمكان التعبئة
...
-- البحث عن أقرب نقطة لبداية الفارم
...
FarmButton.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        ...
        if player.Character and player.Character:FindFirstChild("HumanoidRootPart") and autoFarm then
            ...
            local employed = isAlreadyEmployed()
            
            for i = startIndex, #waypoints do
                if not autoFarm then break end
                
                -- التجاوز الذكي للنقطة 5 إذا كان اللاعب متوظفاً بالأساس
                if i == 5 and employed then
                    continue
                end
                ...
                elseif i == 7 then
                    -- النقطة 7: البدء بتتبع السهم بالذكاء الاصطناعي إلى مكان التعبئة
                    task.wait(0.2)
                    
                    -- محاولة التقاط السهم والمشي لنهايته
                    local arrowTarget = findArrowDestination()
                    if arrowTarget then
                        moveToPosition(hrp, arrowTarget, speed)
                        task.wait(0.2)
                        -- التعبئة المباشرة عند الوصول
                        pressEKey()
                    end
                end
