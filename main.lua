...
Title.Text = "قائمة التحكم (التوجيه المباشر)"
...
-- النقاط الست للممرات (بدايات ونهايات الممرات الثلاثة) ليختار منها الممر المطلوب فقط
local aislePoints = {
...
-- اختيار أقرب إحداثي من الممرات الستة بناءً على موقع الرف المطلوب
    ...
    task.spawn(function()
        ...
        while autoFarm do
            if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                ...
                -- 2. الفحص الأهم: هل معها صناديق؟ إذا لا، تروح لنقطة 6 وتأخذ صناديق
                ...
                if playerHasBox() then
                    ...
                    if realDeliveryPoint then
                        -- اختيار نقطة الممر الصحيحة (من الـ 6 نقاط) اللي تخص الرف هذا بالذات
                        local targetAislePoint = getClosestAislePoint(realDeliveryPoint)
                        
                        -- التوجه مباشرة لنقطة ذلك الممر المحدد فقط بدون المرور بباقي الممرات
                        moveToPosition(hrp, targetAislePoint, speed)
                        if not autoFarm then break end
                        task.wait(0.2)
                        
                        -- تفريغ البضاعة في المكان
                        for _ = 1, 12 do
