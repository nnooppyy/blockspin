...
FarmContent.CanvasSize = UDim2.new(0, 0, 0, 160)
...
-- قسم معلومات الفلوس (ATM / Cash) فوق زر الفارم
local MoneyInfoContainer = Instance.new("Frame")
MoneyInfoContainer.Parent = FarmContent
MoneyInfoContainer.Size = UDim2.new(0.95, 0, 0, 50)
MoneyInfoContainer.BackgroundColor3 = Color3.fromRGB(45, 45, 55)
MoneyInfoContainer.BackgroundTransparency = 0.3
MoneyInfoContainer.ZIndex = 52

local MoneyInfoCorner = Instance.new("UICorner")
MoneyInfoCorner.CornerRadius = UDim.new(0, 6)
MoneyInfoCorner.Parent = MoneyInfoContainer

local AtmIcon = Instance.new("ImageLabel")
AtmIcon.Parent = MoneyInfoContainer
AtmIcon.Size = UDim2.new(0, 36, 0, 36)
AtmIcon.Position = UDim2.new(0, 8, 0.5, -18)
AtmIcon.BackgroundTransparency = 1
AtmIcon.Image = "rbxassetid://6023426915"
AtmIcon.ZIndex = 53

local AtmTextLabel = Instance.new("TextLabel")
AtmTextLabel.Parent = MoneyInfoContainer
AtmTextLabel.Size = UDim2.new(1, -50, 0.5, 0)
AtmTextLabel.Position = UDim2.new(0, 50, 0, 4)
AtmTextLabel.BackgroundTransparency = 1
AtmTextLabel.Text = "Bank: 0"
AtmTextLabel.TextColor3 = Color3.fromRGB(100, 220, 100)
AtmTextLabel.TextSize = 11
...
local waypoints = {
    Vector3.new(-445.96, 254.62, 295.86), -- نقطة 1
...
----------------------------------------------------
-- جلب الفلوس الفعلية من اللعبة تلقائياً (Cash & Bank)
----------------------------------------------------
task.spawn(function()
    while true do
        pcall(function()
            ...
            if leaderstats then
                for _, stat in ipairs(leaderstats:GetChildren()) do
                    ...
                    if name == "cash" or name == "money" or name == "wallet" then
                        cashVal = stat.Value
                    elseif name == "bank" or name == "account" then
                        bankVal = stat.Value
                    end
                end
            end
            
            if cashVal == 0 or bankVal == 0 then
                for _, valObj in ipairs(LocalPlayer:GetDescendants()) do
                    if valObj:IsA("IntValue") or valObj:IsA("NumberValue") then
                        local name = string.lower(valObj.Name)
                        if (name == "cash" or name == "money" or name == "wallet") and cashVal == 0 then
                            cashVal = valObj.Value
                        elseif (name == "bank" or name == "account") and bankVal == 0 then
                            bankVal = valObj.Value
                        end
                    end
                end
            end
            
            CashTextLabel.Text = "Cash: " .. tostring(cashVal)
            AtmTextLabel.Text = "Bank: " .. tostring(bankVal)
        end)
        task.wait(1)
    end
end)
            ...
            while autoFarm do
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if not isInsideGrocery(hrp.Position) then
                        ...
                        local distToStart = (hrp.Position - customStartPoint).Magnitude
