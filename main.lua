...
-- القائمة الرئيسية (بشكل مربع كبير ومرتب)
...
----------------------------------------------------
-- التقسيم الداخلي: القائمة الجانبية (يسار) + محتوى (يمين)
----------------------------------------------------

-- القائمة الجانبية اليسرى (أزرار التنقل: Farm و Settings)
...
-- زر الفرام في الجانب الأيسر
local TabFarmButton = Instance.new("TextButton")
TabFarmButton.Parent = Sidebar
TabFarmButton.Size = UDim2.new(0.9, 0, 0, 38)
TabFarmButton.BackgroundColor3 = Color3.fromRGB(45, 45, 65) -- لون نشط افتراضياً
TabFarmButton.Text = "📦 Farm"
TabFarmButton.TextColor3 = Color3.fromRGB(255, 255, 255)
TabFarmButton.TextSize = 13
...
----------------------------------------------------
-- 1. محتوى قسم الفرام (يظهر في اليمين)
----------------------------------------------------
...
SpeedInput.Parent = FarmContent
SpeedInput.Size = UDim2.new(0.95, 0, 0, 36)
...
ShowPointsButton.Parent = FarmContent
...
----------------------------------------------------
-- 2. محتوى قسم الإعدادات (يظهر في اليمين بشكل عرضي)
----------------------------------------------------
...
-- زر نسخ رابط السيرفر (شكل عرضي فخم)
local CopyLinkButton = Instance.new("TextButton")
CopyLinkButton.Parent = SettingsContent
CopyLinkButton.Size = UDim2.new(0.95, 0, 0, 42)
CopyLinkButton.Position = UDim2.new(0, 0, 0, 10)
...
----------------------------------------------------
-- التبديل بين الأقسام (عند الضغط في اليسار يتغير اليمين)
----------------------------------------------------
TabFarmButton.MouseButton1Click:Connect(function()
    FarmContent.Visible = true
    SettingsContent.Visible = false
    TabFarmButton.BackgroundColor3 = Color3.fromRGB(45, 45, 65)
    TabFarmButton.TextColor3 = Color3.fromRGB(255, 255, 255)
    TabSettingsButton.BackgroundColor3 = Color3.fromRGB(30, 30, 40)
    TabSettingsButton.TextColor3 = Color3.fromRGB(180, 180, 200)
end)
...
TabSettingsButton.MouseButton1Click:Connect(function()
    ...
    TabSettingsButton.BackgroundColor3 = Color3.fromRGB(45, 45, 65)
    TabSettingsButton.TextColor3 = Color3.fromRGB(255, 255, 255)
    TabFarmButton.BackgroundColor3 = Color3.fromRGB(30, 30, 40)
...
-- فحص هل الصندوق في اليد، وإذا طاح يرجع يشيله تلقائياً
                ...
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    while autoFarm and playerHasBox() do
                        ...
                        if realDeliveryPoint then
                            ...
                            -- تعديلك هنا: إذا تم ضربه وطلع برا الدائرة/النص، يرجع فوراً للنص ويتأكد أن الصندوق في يده ويستمر
                            while autoFarm and isTargetStillActive(realDeliveryPoint) do
                                checkAndUnsit(humanoid)
                                ensureBoxEquipped()
                                
                                -- فحص المسافة: إذا ابتعد أكثر من 4 متر عن نقطة التوصيل (بسبب التبقيص)، يرجع يوقف في النص مباشرة
                                if (hrp.Position - realDeliveryPoint).Magnitude > 4 then
                                    moveToPosition(hrp, Vector3.new(realDeliveryPoint.X, hrp.Position.Y, realDeliveryPoint.Z), speed)
                                end
