...
-- ضغط الرقم 2 لاستخدام الصندوق إذا لم يكن مستعملاً
                    ...
                    if not isDepositing then
                        ...
                        while autoFarm and playerHasBox() and not isDepositing do
                            ...
                            if realDeliveryPoint then
                                ...
                                -- أول وصول لوسط الدائرة: تفعيل الصندوق برقم 2 إذا لم يكن مستعملاً
                                task.wait(0.05)
                                ...
                                -- حلقة تفقد أثناء التعبئة (تتعامل مع التبقيس وخروج اللاعب من الدائرة)
                                while autoFarm and isTargetStillActive(realDeliveryPoint) and not isDepositing do
                                    checkAndUnsit(humanoid)
                                    if autoDepositEnabled and getPlayerCash() >= (tonumber(DepositInputBox.Text) or targetDepositAmount) and not hasDepositedThisCycle then break end
                                    
                                    -- **التعديل الجديد:** فحص إذا طار أو انضرب وطلع برا الدائرة (المسافة أكبر من 3.5 متر عن نقطة الوسط)
                                    ...
                                    if distFromCenter > 3.5 then
                                        -- إذا طلع برا الدائرة بسبب التبقيس، يرجعه فوراً لوسط الدائرة
                                        hrp.CFrame = CFrame.new(currentPos)
                                        task.wait(0.1)
                                        -- يتأكد هل هو ماسك الصندوق أو مستخدمه، إذا لا يضغط 2
                                        if not isUsingBoxActive() then
                                            pressTwoKey()
                                            task.wait(0.1)
                                        end
