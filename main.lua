...
----------------------------------------------------
-- منطق الـ Auto Respawn (الضغط فقط عند اختفاء الأرقام وصلاحية الزر)
----------------------------------------------------
            ...
            if pGui then
                for _, descendant in ipairs(pGui:GetDescendants()) do
                    if (descendant:IsA("TextButton") or descendant:IsA("TextLabel")) and descendant.Visible then
                        ...
                        -- التحقق من وجود كلمة respawn وأنه لا توجد أرقام عداد بداخل النص (أي انتهى العداد واختفت الأرقام)
                        if string.find(text, "respawn") and not string.find(text, "%d") then
                            ...
                            if targetButton and targetButton.Visible then
                                ...
                                local centerY = abPos.Y + (abSize.Y / 2)
                                
                                -- الضغط مرة واحدة في المنتصف تماماً
