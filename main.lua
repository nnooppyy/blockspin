...
-- فحص لون النص أو الزر في الشاشة (هل هو أحمر أم أخضر)
local function isEmployedByColor()
    ...
    if playerGui then
        for _, gui in ipairs(playerGui:GetDescendants()) do
            if gui:IsA("TextLabel") or gui:IsA("TextButton") or gui:IsA("Frame") then
                if gui.Visible then
                    -- فحص درجة اللون الأحمر في النص أو الخلفية
                    local textColor = gui:IsA("TextLabel") or gui:IsA("TextButton") and gui.TextColor3 or nil
                    local bgColor = gui.BackgroundColor3
                    
                    -- إذا كان اللون يميل للأحمر (نسبة الأحمر مرتفعة والأخضر منخفضة)
                    if textColor and (textColor.R > 0.6 and textColor.G < 0.4) then
                        return true
                    elseif bgColor and (bgColor.R > 0.6 and bgColor.G < 0.4) then
                        return true
                    end
                end
            end
...
local function findArrowDestination()
    ...
    -- 1. البحث عن Beam (شريط السهم الممتد في اللعبة)
    ...
    -- 2. البحث عن مجسمات أو أسهم مؤشرة
    ...
    task.spawn(function()
        ...
        if player.Character and player.Character:FindFirstChild("HumanoidRootPart") and autoFarm then
            ...
            for i = startIndex, #waypoints do
                ...
                if i == 5 then
                    -- النقطة 5: فحص اللون (أحمر أم أخضر)
                    task.wait(0.3)
                    local employed = isEmployedByColor()
                    
                    if not employed then
                        -- إذا كان لون الكلام أخضر (غير متوظف) يقوم بالتوظيف
                        clickSlightlyBelowCenter()
                        task.wait(0.3)
                    end
                    -- إذا كان أحمر (متوظف) يتجاهل الضغط ويكمل مباشرة للنقطة 6
