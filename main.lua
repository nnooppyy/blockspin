...
ToggleCircleButton.MouseButton1Click:Connect(function()
    ...
    if scriptEnabled then
        ToggleCircleButton.BackgroundColor3 = Color3.fromRGB(50, 200, 50)
...
local function moveToPosition(hrp, targetPos, speed)
    if not autoFarm or not scriptEnabled then return end
    ...
    repeat
        task.wait(0.05)
    until (tick() - start >= tweenTime) or not autoFarm or not scriptEnabled
    
    if not autoFarm or not scriptEnabled then
        tween:Cancel()
    end
end
...
FarmButton.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        ...
        while autoFarm and scriptEnabled do
            if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if status == "unemployed" or status == "unknown" then
                    ...
                    for i = startIndex, 6 do
                        if not autoFarm or not scriptEnabled then break end
                ...
                if not playerHasBox() then
                    ...
                    for _ = 1, 10 do
                        if not autoFarm or not scriptEnabled or playerHasBox() then break end
                        pressEKey()
                        task.wait(0.2)
                    end
                    task.wait(0.3)
                ...
                if playerHasBox() then
                    ...
                    if realDeliveryPoint then
                        ...
                        while autoFarm and scriptEnabled and isTargetStillActive(realDeliveryPoint) do
                            task.wait(0.5)
                        end
