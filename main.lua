...
----------------------------------------------------
local customStartPoint = Vector3.new(-67.63, 254.62, 550.38) -- نقطة صفر
...
----------------------------------------------------
-- منطق التنقل والحركة (Farm Logic المشروط بالنقطة 0 و 1)
----------------------------------------------------
            ...
            while autoFarm do
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if not isInsideGrocery(hrp.Position) then
                        -- التحقق من القرب المباشر من النقطة 0 أو النقطة 1 فقط
                        local distToPoint0 = (hrp.Position - customStartPoint).Magnitude
                        local distToPoint1 = (hrp.Position - waypoints[1]).Magnitude
                        local proximityThreshold = 40 -- مدى القرب المطلوب للاستجابة

                        local shouldRunPath = false
                        
                        if distToPoint0 <= proximityThreshold then
                            -- لو قريب من نقطة صفر، يذهب إليها أولاً ثم يمر بـ 1، 2، 3، 4
                            moveToPosition(hrp, customStartPoint, speed)
                            shouldRunPath = true
                        elseif distToPoint1 <= proximityThreshold then
                            -- لو قريب من نقطة واحد، يبدأ فوراً التسلسل 1، 2، 3، 4
                            shouldRunPath = true
                        end

                        if shouldRunPath then
