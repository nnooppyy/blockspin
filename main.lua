...
----------------------------------------------------
-- منطق التنقل والحركة وإيداع الأموال (بسرعة عالية)
----------------------------------------------------
...
function moveToPositionHelper(hrp, targetPos, speed)
    ...
    while isTeleportDetected() and autoFarm do task.wait(0.1) end
...
-- دالة الضغط على زر رقم 2 في الكيبورد
local function pressKeyTwo()
...
-- التحقق عما إذا كان هناك صندوق نشط ومستخدم حالياً
local function isBoxActiveOrEquipped()
    if LocalPlayer.Character then
        for _, child in ipairs(LocalPlayer.Character:GetChildren()) do
...
local function executeDepositSequence(speed)
    ...
    -- جلب الرصيد الحالي بالكامل لإيداعه
    ...
    -- الضغط على زر التأكيد للإيداع
    clickAtScreenPosition(366, 196)
    task.wait(0.3)
    
    -- العودة السريعة من الطريق المعاكس
    moveToPositionHelper(hrp, aislePoints[3], speed)
    moveToPositionHelper(hrp, waypoints[9], speed)
end
        ...
        task.spawn(function()
            while autoFarm do
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    checkAndUnsit(humanoid)
                    local speed = 35 -- سرعة حركة أعلى ومستقرة

                    -- فحص المبلغ: إذا وصل للحد أو تخطاه، يتم إيداع كل الفلوس مرة واحدة وبسرعة
                    local currentCash = getPlayerCash()
                    ...
                    if not isDepositing then
                        if not isInsideGrocery(hrp.Position) then
                            ...
                            if distToPreStart <= proximityThreshold or distToPoint0 <= proximityThreshold then
                                ...
                                if autoFarm then task.wait(0.1) clickCenter() task.wait(0.3) end
                        ...
                        while autoFarm and playerHasBox() and not isDepositing do
                            ...
                            -- التأكد قبل البيع: إذا لم يكن الصندوق مستخدماً/مجهزاً، اضغط على الرقم 2
                            if not isBoxActiveOrEquipped() then
                                pressKeyTwo()
                                task.wait(0.1)
                            end
