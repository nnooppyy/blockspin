...
-- زر الدائرة العائم (بتصميم Zunex البنفسجي الفخم)
...
local waypoints = {
    ...
    Vector3.new(160.40, 254.86, 250.37), -- 3 (نقطة الشارع الرئيسية)
    Vector3.new(160.37, 255.18, 227.71), -- 4
    Vector3.new(159.49, 254.99, 204.32), -- 5
    Vector3.new(164.69, 254.99, 202.29), -- 6 (التوظيف)
    Vector3.new(148.91, 254.74, 208.63), -- 7 (مكان الصناديق)
    Vector3.new(125.54, 254.74, 201.89), -- 8 (ممر الرفوف الأساسي)
}
...
-- التحكم بفتح وإغلاق القائمة عبر زر الدائرة العائم
...
-- الانتقال مع الإيقاف المؤقت أثناء ظهور رسالة الحماية
local function moveToPosition(hrp, targetPos, speed)
    ...
    repeat
        ...
        if isTeleportDetected() then
            ...
            if autoFarm then tween:Play() end
        end
...
-- فحص التوظيف (يبحث عن leave للمتوظف أو ap/apply للعاطل)
local function checkEmploymentStatusText()
    ...
    if playerGui then
        for _, gui in ipairs(playerGui:GetDescendants()) do
            if (gui:IsA("TextLabel") or gui:IsA("TextButton")) and gui.Visible then
                local text = string.lower(gui.Text or "")
                if string.find(text, "leave") then return "employed" end
                if string.find(text, "apply") or string.find(text, "ap") then return "unemployed" end
            end
        end
    end
    return "unknown"
...
-- جلب أقرب نقطة من نقاط الشارع الأولى (1 إلى 3)
    ...
    task.spawn(function()
        while autoFarm do
            ...
            if localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                -- إجبار الشخصية على القيام إذا جلست بالغلط
                ...
                local isInsideStore = (hrp.Position - waypoints[8]).Magnitude < 80
                
                if not playerHasBox() then
                    if status == "unemployed" or status == "unknown" then
                        -- 1. الذهاب إلى أقرب نقطة شارع ثم الوصول إلى نقطة 3
                        ...
                        -- 2. من نقطة 3، البحث عن أقرب نقطة نحو 6 (بين 4 و 5 و 6) والمشي إليها
                        if autoFarm then
                            local nextIndex = 4
                            local minDistance = math.huge
                            for i = 4, 6 do
                                local dist = (hrp.Position - waypoints[i]).Magnitude
                                if dist < minDistance then
                                    minDistance = dist
                                    nextIndex = i
                                end
                            ...
                            for i = nextIndex, 6 do
                                if not autoFarm then break end
                                moveToPosition(hrp, waypoints[i], speed)
                            end
                            
                            -- الضغط للتوظيف عند نقطة 6
                            if autoFarm then
                                task.wait(0.3)
                                clickCenter()
                                task.wait(0.5)
                        ...
                        -- 3. الانتقال إلى نقطة الصناديق (نقطة 7)
                        if autoFarm then
                            moveToPosition(hrp, waypoints[7], speed)
                        end
                    else
                        -- إذا كان اللاعب متوظفاً
                        if not isInsideStore then
                            -- إذا كان خارج المحل، يمر بالنقطة 3 ثم يختار أقرب نقطة نحو 7
                            local startIndex = getClosestStartPoint(hrp.Position)
                            for i = startIndex, 3 do
                                if not autoFarm then break end
                                moveToPosition(hrp, waypoints[i], speed)
                            ...
                            if autoFarm then
                                ...
                                for i = nextIndex, 7 do
                                    if not autoFarm then break end
                                    moveToPosition(hrp, waypoints[i], speed)
                                end
                            end
                        else
                            -- إذا كان داخل المحل، يتجه مباشرة إلى الصناديق (نقطة 7)
                            moveToPosition(hrp, waypoints[7], speed)
                        end
                    end
