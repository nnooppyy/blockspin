...
-- 1. زر "فرام صناديق" (مشي تلقائي بدلاً من الانتقال)
        ...
        while autoFarm do
            ...
            if player.Character and player.Character:FindFirstChild("Humanoid") then
                ...
                for _, point in ipairs(waypoints) do
                    ...
                    -- جعل الشخصية تمشي نحو النقطة
                    humanoid:MoveTo(point)
                    
                    -- الانتظار حتى تصل الشخصية للهدف أو ينتهي الوقت المخصص (8 ثوانٍ لكل مرحلة)
                    local reached = false
                    local connection
                    ...
                    repeat
                        task.wait(0.1)
                    until reached or (tick() - startTime > 8) or not autoFarm
                    
                    if connection then connection:Disconnect() end
                    task.wait(0.5) -- توقف بسيط عند كل مرحلة
                end
            else
                task.wait(0.5)
            end
