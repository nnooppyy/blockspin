...
-- دالة التحرك (تم التعديل لمنع النزول تحت الأرض)
local function moveToPosition(hrp, targetPos, speed)
    if not autoFarm then return end
    
    -- دمج إحداثيات X و Z للهدف مع ارتفاع Y للاعب لضمان المشي على الأرض فقط
...
FarmButton.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        local player = game.Players.LocalPlayer
        local firstRun = true -- لتحديد ما إذا كانت هذه أول دورة
        
        -- التكرار اللانهائي (لن يتوقف حتى تطفيه بنفسك)
        while autoFarm do
            if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                local hrp = player.Character.HumanoidRootPart
                local speed = tonumber(SpeedInput.Text) or 25
                
                -- في الدورة الأولى يبدأ من أقرب نقطة، وبعدها يبدأ من نقطة 1 عشان يعيد الطريق
                local startIndex = 1
                if firstRun then
                    startIndex = getClosestWaypointIndex(hrp.Position)
                    firstRun = false
                end
                ...
                -- إذا خلص يريح ثانية قبل ما يعيد الدورة من جديد
                if autoFarm then
                    task.wait(1)
                end
            else
                task.wait(1) -- إذا مات اللاعب ينتظر ثانية
            end
        end
    end)
end)
