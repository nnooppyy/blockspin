...
----------------------------------------------------
-- قائمة الإحداثيات الكاملة (مع دمج النقاط الثلاث الجديدة بالبداية)
----------------------------------------------------
local waypoints = {
...
-- تحديد أقرب نقطة بداية من النقاط الثلاث الأولى ومتابعة الترتيب
            ...
            while autoFarm do
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if not isInsideGrocery(hrp.Position) then
                        ...
                        -- المشي عبر النقاط الثلاث الأولى بالترتيب (ابتداءً من الأقرب وحتى الأخيرة بينهم)
                        for i = startIndex, 3 do
                            if not autoFarm then break end
                            moveToPosition(hrp, waypoints[i], speed)
                        end
                        -- إكمال باقي نقاط المسار داخل المتجر
