...
local hasCheckedEmployment = false -- متغير لمنع تكرار التحقق من التوظيف
        ...
        while autoFarm do
            ...
            if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if not playerHasBox() then
                    -- فحص التوظيف مرة واحدة فقط عند البداية أو أول تشغيل
                    if not hasCheckedEmployment then
                        ...
                        -- بعد أول مرة، يتجه مباشرة للنقطة 7 (الصناديق) بدون الذهاب لنقطة 6 أبداً
                        moveToPosition(hrp, waypoints[7], speed)
                    end
                ...
                if playerHasBox() then
                    ...
                    if realDeliveryPoint then
                        ...
                        -- الرجوع الطبيعي والسلس بعد التسليم: من الممر (8) ثم إلى نقطة الصناديق (7) مباشرة دون اختراق الجدران أو الذهاب لنقطة 6
                        moveToPosition(hrp, waypoints[8], speed)
                        if not autoFarm then break end
                        moveToPosition(hrp, waypoints[7], speed)
                        if not autoFarm then break end
