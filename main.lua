...
----------------------------------------------------
-- 3. محتوى قسم الأخرى (Other) مع زر Auto Respawn
----------------------------------------------------
...
----------------------------------------------------
-- منطق الـ Auto Respawn (الانتظار 9 ثوانٍ ثم الضغط، وإذا لم تختفِ الكلمة يعيد المحاولة)
----------------------------------------------------
...
task.spawn(function()
    ...
    while true do
        ...
        if autoRespawnEnabled then
            ...
            -- إذا ظهرت الكلمة، نبدأ حلقة مراقبة وضغط حتى تختفي تماماً
            if respawnFound then
                while autoRespawnEnabled do
                    -- التحقق من وجود الكلمة قبل بدء الـ 9 ثوانٍ
                    local checkBefore = false
                    ...
                    if not checkBefore then break end -- إذا اختفت، نخرج من الحلقة
                    
                    -- انتظار 9 ثوانٍ مع التحقق المستمر خلالها
                    local waitTimer = 0
                    local aborted = false
                    
                    while waitTimer < 9 and autoRespawnEnabled do
                        task.wait(0.5)
                        ...
                        -- إذا اختفت الكلمة أثناء الـ 9 ثوانٍ، نوقف العد ونلغي هذه المحاولة
                    ...
                    -- إذا انتهت الـ 9 ثوانٍ ولم تختفِ، نقوم بالضغط
                    if not aborted and autoRespawnEnabled then
                        ...
                        -- ننتظر قليلاً لنرى هل اختفت الكلمة بعد الضغط أم لا
                        task.wait(1)
                    else
                        break
                    end
