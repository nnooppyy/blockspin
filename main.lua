...
-- عرض الكاش والبنك فوق في شريط العنوان بشكل مرتب ودقيق
local CashTextLabel = Instance.new("TextLabel")
CashTextLabel.Parent = TopBar
CashTextLabel.Size = UDim2.new(0, 100, 1, 0)
CashTextLabel.Position = UDim2.new(0, 10, 0, 0)
...
CashTextLabel.ZIndex = 52

local AtmTextLabel = Instance.new("TextLabel")
AtmTextLabel.Parent = TopBar
AtmTextLabel.Size = UDim2.new(0, 100, 1, 0)
AtmTextLabel.Position = UDim2.new(0, 110, 0, 0)
...
----------------------------------------------------
-- منطق التنقل والحركة (Farm Logic الذكي والمعدل)
----------------------------------------------------
            ...
            while autoFarm do
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if not isInsideGrocery(hrp.Position) then
                        -- البحث عن أقرب نقطة في المسار ليبدأ منها اللاعب مباشرة أينما كان
                        local closestIndex = 1
                        local minDst = math.huge
                        for i, pos in ipairs(waypoints) do
                            local dst = (hrp.Position - pos).Magnitude
                            if dst < minDst then
                                minDst = dst
                                closestIndex = i
                            end
                        ...
                        -- التحرك من أقرب نقطة تم رصدها حتى نهاية المسار
                        for i = closestIndex, #waypoints - 2 do
...
----------------------------------------------------
-- جلب وتحديث الكاش والبنك الحقيقي تلقائياً ودون تخريط
----------------------------------------------------
task.spawn(function()
    while true do
        pcall(function()
            ...
            -- فحص الـ Attributes الأساسية للعبة
            ...
            -- فحص مجلدات الـ leaderstats والبيانات الرسمية
            local searchFolders = {"leaderstats", "Data", "PlayerData", "Stats"}
                ...
                if folder then
                    for _, child in ipairs(folder:GetChildren()) do
                        ...
                        if (cName == "cash" or cName == "money" or cName == "wallet") and not cashVal then
                            cashVal = child.Value
                        elseif (cName == "bank" or cName == "balance") and not bankVal then
                            bankVal = child.Value
                        end
                    end
                end
            ...
            -- تحديث القراءات في شريط العنوان فوق بدقة
