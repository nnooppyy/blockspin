...
local waypoints = {
    ...
    Vector3.new(136.50, 255.47, 203.46), -- 8 (نقطة وسيطة لضمان المشي بالمسار)
...
-- دالة المشي الآمنة التي تتوقف عند التيليبرورت وتكمل بدقة من حيث توقفت بدون شطح
        ...
        task.spawn(function()
            ...
            while autoFarm do
                ...
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    -- الذهاب لمكان الصناديق عبر المسار الصحيح
                    ...
                    -- إذا معه صندوق، يمشي بالمسار حصراً (7 -> 8 -> 9 -> الرفوف)
                    if playerHasBox() then
                        ...
                        if realDeliveryPoint then
                            ...
                            -- الرجوع بنفس المسار بالترتيب العكسي الدقيق (9 -> 8 -> 7)
                            moveToPosition(hrp, waypoints[9], speed)
                            if not autoFarm then break end
