...
-- قائمة الإحداثيات السبع
local waypoints = {
    Vector3.new(61.82, 254.96, 56.74),   -- النقطة 1 (مش فقط)
    Vector3.new(47.26, 254.76, 263.28),  -- النقطة 2 (مشي فقط)
    Vector3.new(160.40, 254.86, 250.37), -- النقطة 3 (مشي فقط)
    Vector3.new(159.49, 254.99, 204.32), -- النقطة 4 (مشي فقط)
    Vector3.new(164.69, 254.99, 202.29), -- النقطة 5 (التوظف - ضغطة ماوس)
    Vector3.new(148.91, 254.74, 208.63), -- النقطة 6 (أخذ الصناديق - ضغط E)
    Vector3.new(125.54, 254.74, 201.89)  -- النقطة 7 (تعبئة - نزول وسهم أسفل وكليك ماوس)
}
...
FarmButton.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        ...
        if player.Character and player.Character:FindFirstChild("HumanoidRootPart") and autoFarm then
            ...
            -- الحركة من أقرب نقطة وحتى النقطة الأخيرة
            for i = startIndex, #waypoints do
                ...
                -- التحكم في الأفعال بحسب رقم النقطة:
                if i == 5 then
                    -- النقطة 5: التوظف عن طريق كليك ماوس أسفل المنتصف
                    task.wait(0.3)
                    clickSlightlyBelowCenter()
                elseif i == 6 then
                    -- النقطة 6: أخذ الصناديق بالضغط على E
                    task.wait(0.2)
                    for _ = 1, 3 do
                        pressEKey()
                        task.wait(0.2)
                ...
                elseif i == 7 then
                    -- النقطة 7: النزول أسفل وسهم أسفل وكليك ماوس
                    task.wait(0.2)
                    ...
                    -- النقاط (1, 2, 3, 4): مجرد التوقف اللحظي والمشي دون إرسال أي كليك أو ضغطة
                    task.wait(0.1)
                end
            ...
            -- التوقف تلقائياً بعد الوصول للنقطة الأخيرة
            if autoFarm then
                autoFarm = false
