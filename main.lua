...
local waypoints = {
    ...
    Vector3.new(125.54, 254.74, 201.89)  -- النقطة 7 (منطقة التعبئة)
}
...
-- دالة فحص هل اللاعب متوظف أم لا
local function isEmployed()
    local player = game.Players.LocalPlayer
    -- يتم الفحص عن طريق الشارات أو الكلمات الخاصة بالوظيفة في واجهة المستخدم
    local playerGui = player:FindFirstChild("PlayerGui")
    if playerGui then
        for _, gui in ipairs(playerGui:GetDescendants()) do
            if gui:IsA("TextLabel") or gui:IsA("TextButton") then
                local text = string.lower(gui.Text or "")
                if string.find(text, "employee") or string.find(text, "موظف") or string.find(text, "مستقيل") then
                    return true
                end
            end
        end
    end
    -- إذا كان يمسك صندوقاً أو لديه أداة عمل
...
-- البحث الذكي عن الهدف المطلوب (Target/Destination)
local function findDynamicTarget()
    -- البحث عن المؤشرات البصرية (BillboardGui / SelectionBox / Highlight) التي تحدد الهدف في اللعبة
    for _, item in ipairs(workspace:GetDescendants()) do
        ...
        elseif item:IsA("Highlight") and item.Adornee and item.Adornee:IsA("BasePart") then
            return item.Adornee.Position
        end
    end
    return nil
end
...
FarmButton.MouseButton1Click:Connect(function()
    ...
    task.spawn(function()
        ...
        if player.Character and player.Character:FindFirstChild("HumanoidRootPart") and autoFarm then
            ...
            local employed = isEmployed()
            
            for i = startIndex, #waypoints do
                if not autoFarm then break end
                
                -- إذا كان متوظفاً، يتم تخطي النقطة 5 تلقائياً
                ...
                -- تنفيذ الأوامر حسب النقطة
                if i == 5 then
                    -- التوظف (في حال لم يكن متوظفاً)
                    task.wait(0.2)
                    clickSlightlyBelowCenter()
                    employed = true
                elseif i == 6 then
                    -- أخذ الصناديق بالضغط على E
                    task.wait(0.2)
                    for _ = 1, 3 do
                        pressEKey()
                        task.wait(0.15)
                ...
                elseif i == 7 then
                    -- النقطة 7: التوجه لمكان التعبئة ثم تفعيل البحث الذكي عن الهدف
                    task.wait(0.2)
                    
                    -- البحث الذكي عن موقع البيع/التعبئة المطلوب
                    local targetPos = findDynamicTarget()
                    if targetPos then
                        moveToPosition(hrp, targetPos, speed)
                    end
                end
            ...
            if autoFarm then
                autoFarm = false
                FarmButton.Text = "تم الوصول للهدف!"
                FarmButton.BackgroundColor3 = Color3.fromRGB(50, 150, 50)
                task.wait(2)
                FarmButton.Text = "فرام صناديق: [معطل]"
            end
