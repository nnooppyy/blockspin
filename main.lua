...
----------------------------------------------------
-- منطق الـ Auto Respawn (الضغط التلقائي على زر الرسبون)
----------------------------------------------------
...
-- حلقة فحص زر الرسبون والضغط عليه تلقائياً
task.spawn(function()
    ...
    while true do
        ...
        if autoRespawnEnabled then
            ...
            if pGui then
                -- البحث عن أي زر أو نص يحتوي على كلمة Respawn
                for _, descendant in ipairs(pGui:GetDescendants()) do
                    if (descendant:IsA("TextButton") or descendant:IsA("TextLabel")) and descendant.Visible then
                        ...
                        if string.find(text, "respawn") then
                            -- إذا وُجد العنصر، نقوم بمحاولة الضغط عليه مباشرة إذا كان زر
                            if descendant:IsA("TextButton") then
                                local abPos = descendant.AbsolutePosition
                                local abSize = descendant.AbsoluteSize
                                ...
                                -- إذا كان TextLabel، نبحث عن الزر الأب له أو نضغط في مكانه
                                local parentBtn = descendant.Parent
                                if parentBtn and parentBtn:IsA("TextButton") then
                                    local abPos = parentBtn.AbsolutePosition
                                    local abSize = parentBtn.AbsoluteSize
                                    local centerX = abPos.X + (abSize.X / 2)
                                    local centerY = abPos.Y + (abSize.Y / 2)
