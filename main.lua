...
-- وظيفة إنشاء النقاط المرئية مع الأرقام
...
----------------------------------------------------
-- دوال التفاعل لاكتشاف الفلوس والواجهة
----------------------------------------------------
local VirtualInputManager = game:GetService("VirtualInputManager")
local GuiService = game:GetService("GuiService")
...
local function getPlayerCash()
    for _, gui in ipairs(PlayerGui:GetDescendants()) do
        if gui:IsA("TextLabel") and gui.Visible then
            ...
            if string.find(txt, "%$") or string.match(txt, "%d+") then
...
local isDepositing = false -- القفل الجديد لمنع التعليق
...
----------------------------------------------------
-- تنفيذ مسار الإيداع (معدلة لمنع التعليق)
----------------------------------------------------
local function executeDepositSequence(speed)
    ...
    -- 3. الذهاب إلى نقطة الإيداع والصراف
    moveToPositionHelper(hrp, autoDepositPoint, speed)
    task.wait(0.5)
    if not autoFarm then return end
    
    -- التفاعل مع الصراف والإيداع
    for _ = 1, 3 do
        pressEKey()
        task.wait(0.1)
    end
        ...
        task.spawn(function()
            while autoFarm do
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    -- ====================================================
                    -- الفحص والإيداع مقفل بـ isDepositing لمنع التكرار والتعليق
                    -- ====================================================
                    if autoDepositEnabled and getPlayerCash() >= targetDepositAmount and isInsideGrocery(hrp.Position) and not isDepositing then
                        isDepositing = true
                        executeDepositSequence(speed)
                        task.wait(2) -- نعطيه مهلة عشان يتحدث رصيد الفلوس باللعبة وتختفي
                        isDepositing = false
                        continue
                    end
                    -- ====================================================
