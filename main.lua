...
MainFrame.Size = UDim2.new(0, 200, 0, 230) -- كبرت الص شوي عشان الزر الجديد
...
Title.Text = "قائمة التحكم الذكية"
...
-- دالة إظهار أو إخفاء النقاط في العالم
local function toggleVisualPoints()
    ...
    if showPointsEnabled then
        ...
        -- دالة فرعية لرسم كرة عند كل إحداثي
        ...
        -- رسم نقاط الطريق العامة (1 إلى 7) باللون الأزرق
        for i, pos in ipairs(waypoints) do
            createMarker(pos, BrickColor.new("Bright blue"), "WP " .. i)
        end
        
        -- رسم نقاط الممرات باللون الأخضر
...
-- فحص هل الشخصية ماسكة صناديق بيدها ولا لا
...
-- البحث عن الرف المطلوب في المتجر
...
-- اختيار أقرب إحداثي من الممرات الستة للعبور الآمن
    ...
    task.spawn(function()
        ...
        while autoFarm do
            if player.Character and player.Character:FindFirstChild("HumanoidRootPart") then
                ...
                -- 1. التأكد من الدخول والتوظف إذا كان اللاعب برا المتجر
                ...
                -- 2. الفحص: هل معها صناديق؟ إذا لا، تروح لنقطة 6 وتأخذ صناديق
                ...
                if playerHasBox() then
                    ...
                    if realDeliveryPoint then
                        ...
                        -- ج) تفريغ البضاعة وهو واقف قدام الرف الصح تماماً
                        for _ = 1, 15 do
                            if not autoFarm or not playerHasBox() then break end
                            pressEKey()
                            task.wait(0.15)
                        end
                    else
