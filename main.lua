...
local waypoints = {
    ...
    Vector3.new(136.50, 255.47, 203.46), -- 8 (النقطة الجديدة المطلوبة للمرور)
    Vector3.new(125.54, 254.74, 201.89), -- 9 (ممر الرفوف)
}
...
local function findHiddenDeliveryTarget()
    ...
    local searchCenter = waypoints[9] -- تم التحديث لتصبح النقطة 9 (ممر الرفوف)
    local targetPos = nil
    local minDist = 120
...
FarmButton.MouseButton1Click:Connect(function()
    ...
    if autoFarm then
        ...
        task.spawn(function()
            -- **خطوة التوظيف تتم مرة واحدة فقط عند تفعيل الفرام**
            ...
            while autoFarm do
                ...
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    if not playerHasBox() then
                        -- يتجه مباشرة لمكان الصناديق
                        moveToPosition(hrp, waypoints[7], speed)
                        if not autoFarm then break end
                        task.wait(0.2)
                        
                        -- ضغط زر E 5 مرات لالتقاط الصندوق
                        for _ = 1, 5 do
                    ...
                    if playerHasBox() then
                        -- يمر بالنقطة الجديدة أولاً (بين الصندوق وممر الرفوف)
                        moveToPosition(hrp, waypoints[8], speed)
                        if not autoFarm then break end

                        -- يكمل لممر الرفوف الأساسي
                        moveToPosition(hrp, waypoints[9], speed)
                        if not autoFarm then break end
                        task.wait(0.2)
                        ...
                        if realDeliveryPoint then
                            ...
                            -- الرجوع الطبيعي بالنقاط: من الممر (9) ثم النقطة الجديدة (8) عشان يرجع للصناديق
                            moveToPosition(hrp, waypoints[9], speed)
                            if not autoFarm then break end
