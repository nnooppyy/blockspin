...
MainFrame.Size = UDim2.new(0, 360, 0, 260) -- زنا شوي الطول عشان الزر الجديد
...
FarmContent.CanvasSize = UDim2.new(0, 0, 0, 230)
...
DepositInputBox.PlaceholderText = "Enter Deposit Amount..."
...
-- زر يدوي لاختبار الإيداع فوراً (عشان نتاكد انه يروح للنقاط الصح)
local TestDepositButton = Instance.new("TextButton")
TestDepositButton.Parent = FarmContent
TestDepositButton.Size = UDim2.new(0.95, 0, 0, 34)
TestDepositButton.BackgroundColor3 = Color3.fromRGB(60, 130, 90)
TestDepositButton.Text = "⚡ Test Deposit Route Now"
TestDepositButton.TextColor3 = Color3.fromRGB(255, 255, 255)
TestDepositButton.TextSize = 11
TestDepositButton.Font = Enum.Font.GothamBold
TestDepositButton.ZIndex = 52

local TestDepositCorner = Instance.new("UICorner")
TestDepositCorner.CornerRadius = UDim.new(0, 6)
TestDepositCorner.Parent = TestDepositButton
...
local aislePoints = {
    ...
    Vector3.new(128.39, 255.32, 167.04)  -- رف 3
}
...
----------------------------------------------------
-- دوال التفاعل مع واجهة اللعبة (الكاش والإيداع)
----------------------------------------------------
local VirtualInputManager = game:GetService("VirtualInputManager")
local GuiService = game:GetService("GuiService")

local function clickUIElement(gui)
...
local function clickButtonByKeyword(keyword)
    for _, gui in ipairs(PlayerGui:GetDescendants()) do
...
local function getPlayerCash()
    for _, gui in ipairs(PlayerGui:GetDescendants()) do
        if gui:IsA("TextLabel") and gui.Visible then
            ...
            if cleanNum and cleanNum > 0 and (string.find(txt, "%$") or cleanNum > 10) then
                return cleanNum
            end
        end
    end
...
----------------------------------------------------
-- دالة تنفيذ مسار الإيداع مباشرة (الترتيب: نقطة 8 ➔ رف 3 ➔ نقطة الإيداع)
----------------------------------------------------
local function executeDepositSequence(speed)
    ...
    -- 2. الذهاب إلى رف 3 (aislePoints[3])
    moveToPositionHelper(hrp, aislePoints[3], speed)
    if not autoFarm then return end
    
    -- 3. الذهاب إلى نقطة الإيداع
    ...
    clickButtonByKeyword("deposit")
    task.wait(0.5)
    
    typeInTextBox(targetDepositAmount)
    task.wait(0.5)
    
    clickButtonByKeyword("deposit")
    clickButtonByKeyword("confirm")
    clickButtonByKeyword("submit")
    clickButtonByKeyword("yes")
end

-- زر الاختبار اليدوي للإيداع
TestDepositButton.MouseButton1Click:Connect(function()
    task.spawn(function()
        TestDepositButton.Text = "⏳ Running Route..."
...
----------------------------------------------------
-- منطق التنقل والحركة (Farm Logic الأساسي)
----------------------------------------------------
...
-- تعريف دالة الحركة بشكل عام لكي تستخدم في الإيداع وخارج الإيداع
function moveToPositionHelper(hrp, targetPos, speed)
    if not autoFarm and TestDepositButton.Text ~= "⏳ Running Route..." then return end
    local humanoid = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Humanoid")
    checkAndUnsit(humanoid)

    while isTeleportDetected() and (autoFarm or TestDepositButton.Text == "⏳ Running Route...") do task.wait(0.2) end
    ...
    repeat
        ...
        if isTeleportDetected() then
            ...
            if autoFarm or TestDepositButton.Text == "⏳ Running Route..." then
        ...
        task.spawn(function()
            while autoFarm do
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    -- ==========================================
                    -- التحقق من الإيداع التلقائي فور توفر الفلوس وأثناء التواجد داخل البقالة
                    -- ==========================================
                    if autoDepositEnabled and getPlayerCash() >= targetDepositAmount and isInsideGrocery(hrp.Position) then
                    ...
                    if not playerHasBox() then
                        moveToPositionHelper(hrp, waypoints[#waypoints - 1], speed)
                        if not autoFarm then break end
                        task.wait(0.3)
                    ...
                    while autoFarm and playerHasBox() do
                        moveToPositionHelper(hrp, waypoints[#waypoints], speed)
                        if not autoFarm then break end
                        task.wait(0.2)
