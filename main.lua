...
-- دالة الحركة الثابتة على المسار مع التوقف عند التيليبرورت
...
-- التحقق هل اللاعب معه صناديق (أدوات/Tools) بيده أو تتبع للأغراض
        ...
        task.spawn(function()
            ...
            while autoFarm do
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    -- 1. إذا ما معه صناديق أبداً، يروح لنقطة 7 (الصناديق) ويجمع أكبر قدر ممكن لين يتعبى
                    if not playerHasBox() then
                        moveToPosition(hrp, waypoints[7], speed)
                        if not autoFarm then break end
                        task.wait(0.3)
                        
                        -- يضغط E عدة مرات عشان يجمع كل الصناديق المتوفرة عنده
                        for _ = 1, 8 do
                    ...
                    -- 2. مادام معه صناديق، يلف ويوزعهم كلهم واحد وراء الثاني بدون ما يرجع لنقطة 7 نهائياً
                    while autoFarm and playerHasBox() do
                        ...
                        if realDeliveryPoint then
                            ...
                            -- يرجع لممر الرفوف (نقطة 8) عشان يشوف الصندوق اللي بعده إذا لسه معه صناديق
                            moveToPosition(hrp, waypoints[8], speed)
                            if not autoFarm then break end
                        else
                            task.wait(1)
