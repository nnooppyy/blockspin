...
-- دالة ذكية للتحقق مما إذا كان اللاعب غير متوظف (زر Apply أخضر)
-- أو متوظف (زر Leave أحمر) ونقر زر التوظيف مرة واحدة فقط عند الحاجة
    ...
    pcall(function()
        ...
        for _, gui in ipairs(PlayerGui:GetDescendants()) do
            if gui:IsA("TextButton") and gui.Visible then
                ...
                -- إذا الزر يخص مغادرة الوظيفة (يعني متوظف حالياً)
                if string.find(text, "leave") or string.find(text, "استقالة") or string.find(text, "ترك") then
                    return -- اللاعب متوظف، لا تقم بأي شيء!
                end
                
                -- إذا الزر يخص التوظيف (أخضر أو يحتوي على Apply)
                if string.find(text, "apply") or string.find(text, "توظيف") or string.find(text, "انضمام") then
                    needsApply = true
                end
            end
        end
        
        -- إذا وجدنا أنه غير متوظف، نضغط مرة واحدة
        if needsApply then
            clickCenter()
            task.wait(0.3)
        end
                    ...
                    if not isDepositing then
                        if not isInsideGrocery(hrp.Position) then
                            ...
                            if distToPreStart <= proximityThreshold or distToPoint0 <= proximityThreshold then
                                ...
                                if autoFarm then 
                                    task.wait(0.1) 
                                    checkAndApplyJobIfNeeded() -- يفحص مرة وحدة عند الوصول للتوظيف
                                    task.wait(0.2) 
                                end
                            ...
                            -- اللاعب "جوا" البقالة وشغل الفارم!
                            -- يمشي عكسي: من 9 إلى 8 ثم 7 (للتوظيف)
                            moveToPositionHelper(hrp, waypoints[9], speed)
                            ...
                            if autoFarm then 
                                task.wait(0.1) 
                                checkAndApplyJobIfNeeded() -- يفحص حالة التوظيف مرة واحدة فقط هنا!
                                task.wait(0.2) 
                            end
                        end
