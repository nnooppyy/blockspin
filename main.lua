...
-- محاكاة ضغط زر الكيبورد للقيام فوراً (زر القفز Space)
local function pressJumpKey()
    VirtualInputManager:SendKeyEvent(true, Enum.KeyCode.Space, false, game)
    task.wait(0.05)
    VirtualInputManager:SendKeyEvent(false, Enum.KeyCode.Space, false, game)
end

-- القيام التلقائي بالمحاكاة
local function forceUnsitWithKeyboard()
...
-- حلقة فحص متواصلة وسريعة للقيام بالأزرار
task.spawn(function()
    while true do
        task.wait(0.05)
        if autoFarm then
            forceUnsitWithKeyboard()
        end
    end
end)
        ...
        while autoFarm do
            ...
            if localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if not playerHasBox() then
                    -- الذهاب إلى التوظيف فقط إذا كان غير متوظف إطلاقاً
                    ...
                    -- يتجه فوراً إلى نقطة الصناديق (7) بدون الرجوع لنقطة 3
                ...
                if playerHasBox() then
                    ...
                    if realDeliveryPoint then
                        ...
                        -- الانتظار أثناء التعبئة مع قيام الكيبورد المستمر
                        ...
                        task.wait(0.2)
                        -- العودة إلى الممر ثم مباشرة إلى الصناديق بدون المرور على 3
                        moveToPosition(hrp, targetAislePoint, speed)
                        task.wait(0.1)
                    else
                        task.wait(0.5)
