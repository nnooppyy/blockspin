...
-- العنوان مع تعديل رابط الديسكورد فوق يسار
...
-- فولدر لإظهار النقاط مع أرقامها في اللعبة عند التفعيل
...
ShowPointsButton.MouseButton1Click:Connect(function()
    ...
    if pointsVisible then
        ...
        -- سيتم عرض الأرقام على النقاط الجديدة
        local waypointsRef = {
        ...
        for index, pos in ipairs(waypointsRef) do
...
----------------------------------------------------
-- المنطق والإحداثيات (تم تحديث النقاط الجديدة هنا)
----------------------------------------------------
local waypoints = {
...
-- اختيار أقرب نقطة بداية بناءً على مكان اللاعب ومتابعة المسار بترتيب النقاط
