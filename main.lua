...
local waypoints = {
    ...
    Vector3.new(146.79, 255.47, 204.95),  -- نقطة 8 المطلوبة
...
local aislePoints = {
    ...
    Vector3.new(128.39, 255.32, 167.04)  -- رف 3 المطلوبة
}
...
----------------------------------------------------
-- دوال قراءة الكاش (المعدلة للبحث في الواجهة مباشرة بناءً على صورتك)
----------------------------------------------------
local VirtualInputManager = game:GetService("VirtualInputManager")
local GuiService = game:GetService("GuiService")
...
-- قراءة الفلوس من واجهة اللعبة (صورة الكاش العلوية اليمنى)
local function getPlayerCash()
    for _, gui in ipairs(PlayerGui:GetDescendants()) do
        if gui:IsA("TextLabel") and gui.Visible then
            local txt = gui.Text or ""
            -- ابحث عن أي نص يحتوي على علامة $ أو أرقام مالية
            if string.find(txt, "%$") or (tonumber(string.gsub(txt, "%D", "")) and string.len(string.gsub(txt, "%D", "")) > 0) then
        ...
        task.spawn(function()
            while autoFarm do
                if LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    ...
                    -- ==========================================
                    -- فحص الإيداع التلقائي (المسار المطلوب: نقطة 8 ➔ رف 3 ➔ إحداثية الإيداع)
                    -- ==========================================
                    if autoDepositEnabled and getPlayerCash() >= targetDepositAmount then
                        if isInsideGrocery(hrp.Position) then
                            ...
                            -- الضغط على زر إيداع / Deposit في القائمة
                            clickButtonByKeyword("deposit")
                            task.wait(0.5)
                            
                            -- كتابة المبلغ تلقائياً
                            typeInTextBox(targetDepositAmount)
                            task.wait(0.5)
                            
                            -- تأكيد العملية بالبحث عن أزرار التأكيد الشائعة
                            clickButtonByKeyword("deposit")
