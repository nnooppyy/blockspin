...
-- التثبت الدقيق من التوظيف بحسب النصوص المشروطة
        ...
        while autoFarm do
            ...
            if localPlayer.Character and localPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                if not playerHasBox() then
                    -- إذا لم يكن متوظفًا (يقرأ كلمة تبدأ بـ ap أو غير معروف كـ employed)
                    if status ~= "employed" then
                        ...
                        -- الانتقال لأقرب نقطة وصولاً لـ 6
                        if autoFarm then
                            local bestNext = 4
                        ...
                        -- إذا كان متوظفًا بالفعل (كلمة leave موجودة)
                        if not isInsideStore then
                            -- خارج المحل: يمر عبر 3 ثم يتجه للصناديق مباشرة
                            local startIndex = getClosestStartPoint(hrp.Position)
                            for i = startIndex, 3 do
                                if not autoFarm then break end
                                moveToPosition(hrp, waypoints[i], speed)
                            ...
                            -- داخل المحل: يتجه مباشرة لنقطة الصناديق (7) بدون 3
                            moveToPosition(hrp, waypoints[7], speed)
                        end
                    end
