-- التأكد من تحميل اللاعب والـ PlayerGui (يتم مرة واحدة عند تشغيل السكربت)
...
FarmButton.MouseButton1Click:Connect(function()
    ...
    if autoFarm then
        ...
        -- ✅ التعديل المطلوب: التحقق من تحميل الشخصية مرة واحدة فقط قبل بدء الحلقة
        ...
        task.spawn(function()
            while autoFarm do
                ...
                -- نستخدم المراجع المخزنة مسبقًا ولا نعيد البحث عن الشخصية
                if not hrp or not humanoid or not hrp.Parent then
                    -- في حال اختفت الشخصية (موت مثلاً) نوقف الحلقة
                    autoFarm = false
                    break
                end
                ...
                if not playerHasBox() then
                    ...
                    -- ضغط زر E 5 مرات لالتقاط الصندوق في النقطة 7 كما طلبته تماماً
                    for _ = 1, 5 do
                ...
                if playerHasBox() then
                    ...
                    if realDeliveryPoint then
                        ...
                        -- الرجوع الطبيعي بالنقاط (بدون اختراق جدران وبدون تيليبرورت): من الممر (8) إلى (7) ثم الفحص الطبيعي عند النقطة (6)
                        moveToPosition(hrp, waypoints[8], speed)
                        if not autoFarm then break end
                        moveToPosition(hrp, waypoints[7], speed)
                        if not autoFarm then break end
